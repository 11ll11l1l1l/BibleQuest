(() => {
  ...
  function addScript(src){return new Promise((resolve,reject)=>{const old=document.querySelector(`script[data-jp-lib="${src}"]`);if(old){if(window.kuromoji)return resolve();if(old.dataset.jpLibState==='error'||old.dataset.jpLibState==='loaded'){old.remove();return addScript(src).then(resolve,reject)}old.addEventListener('load',resolve,{once:true});old.addEventListener('error',reject,{once:true});return}const el=document.createElement('script');el.src=src;el.dataset.jpLib=src;el.dataset.jpLibState='loading';el.onload=()=>{el.dataset.jpLibState='loaded';resolve()};el.onerror=()=>{el.dataset.jpLibState='error';el.remove();reject(new Error('Japanese reading engine could not load'))};document.head.appendChild(el)})}
