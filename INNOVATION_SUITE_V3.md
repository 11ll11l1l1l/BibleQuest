...
1. **Guided Study** — already migrated and owned by `src/app/study.js` (`createGuidedStudyService`) under a separate milestone; already owned elsewhere. Out of #83 scope.
2. **Characters & Places** — already migrated as the Character Detective game mode, owned by `src/app/games.js` / `src/features/games/detectives.js` under a separate milestone; already owned elsewhere. Out of #83 scope.
