...
- Exact release verification: run `34560522189`, fresh attempt 4 job `103161472966`, **success** on exact SHA `77bd0772cb002371cb3ddaa57cf51cd2bea6b7ac`.
...
- Fresh production-live attempt 3 on run `34560806166`, job `103162183940`, re-confirmed both hosts byte-matched the frozen r3 product files and passed canonical Home/Account/navigation, Reader, Games, Transform, Accessibility, and PWA install. It then hit the already-known legacy `waitForFunction` offline-test polling race, so later steps in that attempt were skipped; this was not treated as product PASS or as a reproduced runtime defect.
- Corrected offline-readiness test commit `952271aaf00e1a6f11280d8926b28399eebe8e4e` is exactly one test-only commit ahead of r3 and changes only `tests/v3-offline-shell-smoke.mjs`. Fresh rerun of run `34561097576`, job `103162758331`, passed the corrected offline regression on both canonical and compatibility production hosts.
- Gap-closing production verifier run `34567697491`, job `103163084659`, **success**: it proved the verifier-only delta from r3 is only `tests/v3-offline-shell-smoke.mjs`; re-confirmed both Cloudflare hosts byte-match the frozen release product files; passed canonical corrected Offline Shell, canonical Offline Bible Packs, compatibility corrected Offline Shell, and compatibility 390px basic-browser navigation.
- Historical production-live run `34560806166` attempt 2 / job `103157415628` remains prior same-SHA end-to-end evidence; the fresh split verification above is the current evidence and does not transfer PASS across a changed product SHA because the product SHA never changed.
...
- The fresh production attempt-3 offline red result was reproduced as the known verifier readiness race and cleared with the already-isolated test-only correction. No runtime P0/P1 defect was reproduced, so production code was not changed.
...
- Both Cloudflare Pages hosts serve the r3 product files used for identity verification; this was freshly re-confirmed by gap-closing run `34567697491`.
- Fresh exact release run `34560522189` attempt 4 / job `103161472966` passed the complete release suite on the deployed product SHA.
- Fresh canonical production browser checks in run `34560806166` attempt 3 passed Home/Account/navigation, Reader, Games, Transform, Accessibility, and PWA install before the legacy offline-test race interrupted that workflow.
- Fresh corrected production checks then passed canonical Offline Shell, canonical Offline Bible Packs, compatibility Offline Shell, and compatibility basic browser in run `34567697491` / job `103163084659`.
...
- The live workflow also contained a separate browser-test readiness polling race. The test-only correction at `952271aaf00e1a6f11280d8926b28399eebe8e4e` replaces the fragile `waitForFunction`/immediate second cache read with bounded explicit polling and diagnostic state. It changes no product/runtime file. Fresh corrected verification passed on both live hosts, and gap-closing run `34567697491` additionally passed Offline Bible Packs plus compatibility basic navigation while re-confirming both hosts still serve the frozen r3 product files.
