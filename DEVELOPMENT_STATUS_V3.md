...
- Exact release verification: run `34560522189`, fresh attempt 5 job `103167094939`, **success** on exact SHA `77bd0772cb002371cb3ddaa57cf51cd2bea6b7ac`.
- Exact release evidence from attempt 5: clean exact-SHA/diff hygiene; Cloudflare deployment gate; 53 accumulated v3 architecture validators; 86 edge/security/static regressions; 68 Playwright browser/mobile regressions; explicit PWA/offline/accessibility/core-browser coverage.
...
- Corrected offline-readiness test commit `952271aaf00e1a6f11280d8926b28399eebe8e4e` changes only `tests/v3-offline-shell-smoke.mjs` relative to r3. Fresh rerun attempt 2 of gap-closing run `34567697491`, job `103167200525`, **success**: verifier-only delta confirmed; both Cloudflare hosts byte-match the frozen release product files; canonical corrected Offline Shell, canonical Offline Bible Packs, compatibility corrected Offline Shell, and compatibility 390px basic-browser navigation all passed.
- Historical production-live run `34560806166` attempt 2 / job `103157415628` and prior fresh attempt 3 / job `103162183940` remain same-SHA evidence; the current evidence is attempt 5 exact release verification plus attempt 4 core production smoke and corrected production verifier attempt 2. PASS has not been transferred across a changed product SHA because the product SHA never changed.
...
- Both Cloudflare Pages hosts serve the frozen r3 product files; this was freshly re-confirmed during production attempt 4 and corrected production verifier attempt 2.
- Fresh exact release run `34560522189` attempt 5 / job `103167094939` passed the complete release suite on the deployed product SHA: deployment gate, 53 architecture validators, 86 edge/security/static regressions, and 68 Playwright browser/mobile regressions with PWA/offline/accessibility/core-browser coverage included.
...
- Fresh corrected production checks in run `34567697491` attempt 2 / job `103167200525` passed canonical Offline Shell, canonical Offline Bible Packs, compatibility Offline Shell, and compatibility basic browser while re-confirming both hosts serve the frozen product files.
