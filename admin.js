(() => {
  ...
  const roleNote=r=>({owner:'Full site authority. Cannot be changed by an admin.',admin:'Can manage users, leaders and pastors. Password recovery stays private to each user.',pastor:'Ministry role. No account-admin or password access.',leader:'Leader tools only. No account-admin or password access.',member:'Regular BibleQuest access.'}[r]||'');
  ...
  function userCard(u){
    ...
    return `<article class="admin-user" data-user="${esc(u.id)}"><div class="admin-user-main"><b>${esc(u.name||'Member')}</b><small>${esc(u.email||'No email')}</small><div class="admin-meta">Joined ${relative(u.createdAt)} · Last sign-in ${relative(u.lastSignInAt||u.lastActiveAt)}</div></div><div class="admin-rolebox"><label>App access<select data-role-user="${esc(u.id)}" ${disableRole?'disabled':''}>${roleOptions(u.role)}</select></label><div class="role-note">${esc(roleNote(u.role))}</div></div><div class="admin-actions"><span class="admin-meta">Password recovery is user-controlled with the private recovery code. Admins cannot view or reset it.</span></div>${(u.memberships||[]).length?`<div class="memberships"><div class="memberships-title">Congregation permissions</div>${u.memberships.map(m=>`<div class="membership-row"><span>${esc(m.congregationName)}</span><label>Role<select data-congregation-role data-user-id="${esc(u.id)}" data-congregation-id="${esc(m.congregationId)}">${congregationOptions(m.role)}</select></label></div>`).join('')}</div>`:''}</article>`;
  }
  function bind(){
