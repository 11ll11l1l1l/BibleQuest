(() => {
  ...
  function roleOptions(current){const allowed=adminRole==='owner'?['member','leader','pastor','admin','owner']:['member','leader','pastor'];return allowed.map(r=>`<option value="${r}" ${r===current?'selected':''}>${roleLabel(r)}</option>`).join('')}
  function congregationOptions(current){const roles=adminRole==='owner'?['member','facilitator','leader','pastor','admin']:['member','facilitator','leader','pastor'];return roles.map(r=>`<option value="${r}" ${r===current?'selected':''}>${r[0].toUpperCase()+r.slice(1)}</option>`).join('')}
  ...
  function render(){
    ...
    root.className='';
    root.innerHTML=`<section class="admin-toolbar"><input class="admin-search" data-admin-search placeholder="Search name, email or role…" value="${esc(term)}"><button class="admin-refresh" data-admin-refresh>Refresh</button></section><section class="admin-summary"><div class="admin-stat"><b>${users.length}</b><span>Users</span></div><div class="admin-stat"><b>${counts.pastor}</b><span>Pastors</span></div><div class="admin-stat"><b>${counts.leader}</b><span>Leaders</span></div><div class="admin-stat"><b>${counts.admin+counts.owner}</b><span>Site admins</span></div></section><section class="admin-users">${filtered.map(userCard).join('')||'<div class="admin-empty">No matching users.</div>'}</section>`;
    bind();
  }
  function userCard(u){
    const isSelf=u.id===session.user.id,lockedByAdmin=adminRole!=='owner'&&['admin','owner'].includes(u.role),disableRole=(isSelf&&u.role==='owner')||lockedByAdmin;
    return `<article class="admin-user" data-user="${esc(u.id)}"><div class="admin-user-main"><b>${esc(u.name||'Member')}</b><small>${esc(u.email||'No email')}</small><div class="admin-meta">Joined ${relative(u.createdAt)} · Last sign-in ${relative(u.lastSignInAt||u.lastActiveAt)}</div></div><div class="admin-rolebox"><label>App access<select data-role-user="${esc(u.id)}" ${disableRole?'disabled':''}>${roleOptions(u.role)}</select></label><div class="role-note">${esc(roleNote(u.role))}</div></div><div class="admin-actions"><button class="admin-reset" data-reset-user="${esc(u.id)}" ${!u.email?'disabled':''}>Send password reset email</button></div>${(u.memberships||[]).length?`<div class="memberships"><div class="memberships-title">Congregation permissions</div>${u.memberships.map(m=>`<div class="membership-row"><span>${esc(m.congregationName)}</span><label>Role<select data-congregation-role data-user-id="${esc(u.id)}" data-congregation-id="${esc(m.congregationId)}">${congregationOptions(m.role)}</select></label></div>`).join('')}</div>`:''}</article>`;
  }
  function bind(){
    ...
    root.querySelectorAll('[data-role-user]').forEach(sel=>sel.addEventListener('change',async e=>{const target=e.currentTarget,user=users.find(x=>x.id===target.dataset.roleUser);if(!user)return;const next=target.value,previous=user.role;if(['admin','owner'].includes(next)&&!confirm(`Grant ${roleLabel(next)} access to ${user.name}? This includes site administration privileges.`)){target.value=previous;return}target.disabled=true;try{await invoke({action:'set_role',targetUserId:user.id,role:next});user.role=next;toast(`${user.name} is now ${roleLabel(next)}.`);render()}catch(err){target.value=previous;target.disabled=false;toast(err.message,true)}}));
    root.querySelectorAll('[data-reset-user]').forEach(btn=>btn.addEventListener('click',async e=>{const target=e.currentTarget,user=users.find(x=>x.id===target.dataset.resetUser);if(!user)return;if(!confirm(`Send a Supabase password-reset email to ${user.email}?`))return;target.disabled=true;target.textContent='Sending…';try{await invoke({action:'send_password_reset',targetUserId:user.id,appUrl:cfg.redirectUrl});toast(`Password reset email sent to ${user.email}.`);target.textContent='Reset email sent'}catch(err){target.disabled=false;target.textContent='Send password reset email';toast(err.message,true)}}));
    root.querySelectorAll('[data-congregation-role]').forEach(sel=>sel.addEventListener('change',async e=>{const el=e.currentTarget,old=(users.find(x=>x.id===el.dataset.userId)?.memberships||[]).find(m=>m.congregationId===el.dataset.congregationId)?.role||'member';el.disabled=true;try{await invoke({action:'set_congregation_role',targetUserId:el.dataset.userId,congregationId:el.dataset.congregationId,role:el.value});const u=users.find(x=>x.id===el.dataset.userId),m=(u?.memberships||[]).find(x=>x.congregationId===el.dataset.congregationId);if(m)m.role=el.value;toast('Congregation role updated.');el.disabled=false}catch(err){el.value=old;el.disabled=false;toast(err.message,true)}}));
  }
