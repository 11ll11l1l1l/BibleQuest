(() => {
  ...
  let client=null,session=null,adminRole='',users=[],lastInviteCode='',lastInviteName='';
  ...
  function siteRoleOptions(current){
    ...
    const options=[];
    if(!allowed.includes(current))options.push(`<option value="${esc(current)}" selected>${esc(roleLabel(current))} · legacy</option>`);
    for(const r of allowed)options.push(`<option value="${r}" ${r===current?'selected':''}>${roleLabel(r)}</option>`);
    return options.join('');
  }
  ...
  function setupPanel(){
    const activeMemberships=memberships(),congregations=new Set(activeMemberships.map(m=>m.congregationId));
    const empty=!congregations.size;
    return `<section class="admin-setup ${empty?'needs-setup':''}"><div class="admin-setup-copy"><small>${empty?'MINISTRY SETUP REQUIRED':'MINISTRY STRUCTURE'}</small><h2>${empty?'Create the first congregation':'Congregation roles are active'}</h2><p>${empty?'BibleQuest currently has users but no congregation. Create one here, then share its invite code. Members can be promoted to Pastor, Leader or Facilitator after they join.':'Pastor, Leader and Facilitator are congregation-scoped roles. Use each member card below to manage ministry responsibility.'}</p></div><form data-admin-create-congregation class="admin-create"><label>Congregation / church name<input name="name" minlength="2" maxlength="100" placeholder="e.g. Your church or fellowship" required></label><button class="admin-action">${empty?'Create congregation':'Create another congregation'}</button></form>${lastInviteCode?`<div class="admin-invite"><small>NEW INVITE FOR ${esc(lastInviteName||'CONGREGATION')}</small><code>${esc(lastInviteCode)}</code><p>Share this code with members. After they join, assign Pastor/Leader/Facilitator roles below.</p></div>`:''}${ministryGuide()}</section>`
  }

  function render(){
    const previousSearch=root.querySelector('[data-admin-search]')?.value||'';
    const term=previousSearch.toLowerCase();
    const filtered=users.filter(u=>`${u.name} ${u.email} ${u.role} ${(u.memberships||[]).map(m=>`${m.congregationName} ${m.role}`).join(' ')}`.toLowerCase().includes(term));
    const ms=memberships(),congregations=new Set(ms.map(m=>m.congregationId));
    const counts={pastor:ms.filter(m=>m.role==='pastor').length,leader:ms.filter(m=>m.role==='leader').length,facilitator:ms.filter(m=>m.role==='facilitator').length};
    root.className='';
    root.innerHTML=`${setupPanel()}<section class="admin-toolbar"><input class="admin-search" data-admin-search placeholder="Search name, email, congregation or role…" value="${esc(previousSearch)}"><button class="admin-refresh" data-admin-refresh>Refresh</button></section><section class="admin-summary"><div class="admin-stat"><b>${users.length}</b><span>Users</span></div><div class="admin-stat"><b>${congregations.size}</b><span>Congregations</span></div><div class="admin-stat"><b>${counts.pastor}</b><span>Pastors</span></div><div class="admin-stat"><b>${counts.leader+counts.facilitator}</b><span>Leaders / facilitators</span></div></section><section class="admin-users">${filtered.map(userCard).join('')||'<div class="admin-empty">No matching users.</div>'}</section>`;
    bind();
  }
  function userCard(u){
    ...
    const legacy=['pastor','leader'].includes(u.role);
    return `<article class="admin-user" data-user="${esc(u.id)}"><div class="admin-user-main"><b>${esc(u.name||'Member')}</b><small>${esc(u.email||'No email')}</small><div class="admin-meta">Joined ${relative(u.createdAt)} · Last sign-in ${relative(u.lastSignInAt||u.lastActiveAt)}</div>${legacy?'<div class="admin-warning">Legacy global ministry role — move this authority to a congregation role.</div>':''}</div><div class="admin-rolebox"><label>Platform access<select data-role-user="${esc(u.id)}" ${disableSite?'disabled':''}>${siteRoleOptions(u.role)}</select></label><div class="role-note">${esc(siteRoleNote(u.role))}</div></div><div class="admin-actions"><span class="admin-meta">Passwords and recovery codes remain private to each user.</span></div>${(u.memberships||[]).length?`<div class="memberships"><div class="memberships-title">Congregation ministry roles</div>${u.memberships.map(m=>{const locked=adminRole!=='owner'&&m.role==='admin';return `<div class="membership-row"><span><b>${esc(m.congregationName)}</b><small>${m.active===false?'Inactive membership':'Active membership'}</small></span><label>Ministry role<select data-congregation-role data-user-id="${esc(u.id)}" data-congregation-id="${esc(m.congregationId)}" ${locked?'disabled':''}>${congregationOptions(m.role)}</select></label></div>`}).join('')}</div>`:'<div class="memberships"><div class="memberships-title">Congregation ministry roles</div><div class="admin-empty compact">Not in a congregation yet. Share the congregation invite code first.</div></div>'}</article>`;
  }
  function bind(){
  ...
  async function createCongregation(e){
    ...
    try{
      const result=await client.functions.invoke('bq-create-congregation',{body:{name}});
      if(result.error)throw new Error(await functionMessage(result.error,'Could not create congregation.'));
      lastInviteCode=String(result.data?.inviteCode||'');lastInviteName=String(result.data?.congregation?.name||name);
      toast(`${lastInviteName} created.`);await loadUsers();
    }catch(err){toast(err.message||String(err),true);if(button)button.disabled=false}
  }
  async function loadUsers(){try{const data=await invoke({action:'list_users',page:1,perPage:200});users=data.users||[];adminRole=data.role||adminRole;roleChip.textContent=roleLabel(adminRole);roleChip.className=`admin-badge role-${adminRole}`;render()}catch(err){message('Could not load admin users',err.message,`<p><a class="admin-back" href="./">Back to BibleQuest</a></p>`)}}
