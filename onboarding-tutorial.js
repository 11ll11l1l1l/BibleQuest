(() => {
  ...
  function screen(kind){
    ...
    if(kind==='daily')actual=cloneActual('.today-journey-card')||cloneActual('.modern-focus');
    ...
    const fallback={
      ...
      games:cards([
        ['🧭','Daily Journey','Five-step daily path'],['🧠','Smart Review','Weak & due questions'],
        ['🎯','Quick Play','10 mixed questions'],['🗣️','Who Said It?','Real BSB verse text'],
        ['➡️','What Happens Next?','Open Bible Stories'],['🧩','Verse Order','Put a verse in order'],
        ['🕵️','Bible Detective','Guess from clues'],['⏳','Timeline','Put events in order'],
        ['🧠','Context Mode','Understand why, not just who']
      ]),
      together:cards([
        ...
        ['🏁','Church Challenges','Short shared goals'],['💞','Couple Journey','Private linked journey'],
      ...
      read:cards([
        ['📚','Bible Reader','BSB · Tagalog ULB · 日本語 · licensed NLT'],['📘','Guided Study','Read → Observe → Understand → Apply'],
        ['אΩ','Hebrew & Greek Context','Lemma, morphology & careful context'],['🗂️','Bible Workspace','Highlights, bookmarks, notes & search'],
        ['🏕️','Story Journey','Illustrated foundational stories'],['🗃️','Recall Decks','Open questions by Bible book']
      ]),
      grow:cards([
        ['🎯','My Mission','Focused next step'],['🗺️','Bible World','Travel Scripture with your avatar'],
  ...
  function featureList(){
    return `<div class="bqt-feature-map">
      <details open><summary>🎮 Play <small>different ways to learn</small></summary><p>Daily Journey · Smart Review · Quick Play · Who Said It? · What Happens Next? · Verse Order · Bible Detective · Characters & Places · Timeline · Context Mode</p></details>
      <details><summary>📖 Read <small>Scripture + study tools</small></summary><p>Bible Reader · BSB · Tagalog ULB · Japanese 口語訳 · licensed external editions · Guided Study · Hebrew & Greek Context · Bible Workspace · Story Journey · Recall Decks</p></details>
  ...
  function style(){void SPRITE}

  function close(markDone=true){
    root?.remove();
    root=null;
    document.body.classList.remove('bqt-open');
    if(markDone){localStorage.setItem(DONE,'1');localStorage.setItem(LEGACY_DONE,'1')}
  ...
  function render(){
    ...
    root.querySelector('[data-bqt-skip]')?.addEventListener('click',()=>{if(recoveryCode&&index===0&&!saved)return;close(false)});
