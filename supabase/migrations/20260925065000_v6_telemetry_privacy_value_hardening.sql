...
-- Replaces the ingestion function so even allowlisted property names cannot
-- carry free-form text, emails, UUIDs, exact Scripture locations, or unbounded numbers.
-- Repeated IF NOT EXISTS indexes below are intentional and safe because this
-- forward migration is derived from the prior hardening definition.
