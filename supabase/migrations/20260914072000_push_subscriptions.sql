...
--
-- The existing in-app Notification Center remains the source of truth. This
-- table stores only browser delivery material and explicit push-channel
-- preferences that cannot be represented safely by existing notification
-- records. Push is opt-in: enabled_categories defaults to an empty array.
...
-- switching; client lifecycle code must remove the old owner before registering
-- the endpoint for a different account.
