...
select results_eq($$select enabled_categories from public.bible_push_subscriptions where id='e1000000-0000-4000-8000-000000000001'::uuid$$,array[array['assignment','calendar']::text[]],'own subscription update persists');
...
select is((select count(*)::integer from (update public.bible_push_subscriptions set user_id='11111111-1111-4111-8111-111111111112'::uuid where id='e2000000-0000-4000-8000-000000000002'::uuid returning 1) s),0,'Member A cannot take over hidden Member B subscription');
