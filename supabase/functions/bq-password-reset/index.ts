...
async function insertCode(admin:ReturnType<typeof adminClient>,userId:string,email:string){const code=recoveryCode(),now=new Date().toISOString(),expires=new Date(Date.now()+10*365.25*24*3600*1000).toISOString();const retired=await admin.from('bible_password_reset_codes').update({used_at:now}).eq('user_id',userId).is('used_at',null);if(retired.error)throw retired.error;const inserted=await admin.from('bible_password_reset_codes').insert({user_id:userId,requested_by:userId,code_hash:await hashRaw(normalizeCode(code)),email_hash:await hashRaw(email.trim().toLowerCase()),expires_at:expires,attempts:0,locked_until:null,last_attempt_at:null});if(inserted.error)throw inserted.error;return {code,expires}}
