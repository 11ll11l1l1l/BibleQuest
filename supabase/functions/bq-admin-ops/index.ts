...
async function auditRequired(a:ReturnType<typeof db>,actor:string,target:string|null,action:string){const r=await a.from('bible_admin_audit_log').insert({actor_id:actor,target_user_id:target,action,detail:{stage:'requested'}});if(r.error)throw new Error('Admin audit unavailable')}
async function health(a:ReturnType<typeof db>){const day=new Date(Date.now()-86400000).toISOString(),tables=['bible_assignments','bible_ministry_messages','bible_polls','bible_media_library','bible_notifications','bible_member_recognitions','bible_ministry_calendar','bible_content_reports','bible_content_decisions'];const counts:Record<string,number>={};for(const t of tables){const r=await a.from(t).select('*',{count:'exact',head:true});if(!r.error)counts[t]=r.count||0}const er=await a.from('bible_client_errors').select('id,user_id,congregation_id,surface,message,app_version,created_at').gte('created_at',day).order('created_at',{ascending:false}).limit(50);if(er.error)throw er.error;return {ops_version:OPS_VERSION,checked_at:new Date().toISOString(),counts,client_errors_24h:er.data||[]}}
async function dashboard(a:ReturnType<typeof db>){
  ...
  const [c,p,as,m,po,se,cal,rec,med,h]=await Promise.all([
    ...
    a.from('bible_assignments').select('id,congregation_id,created_by,title,instructions,assignment_type,target_scope,target_id,due_at,points,active,created_at,schedule_at,recurrence_rule,required_reflection,min_quiz_score,evidence_type').gte('created_at',since).order('created_at',{ascending:false}).limit(100),
    a.from('bible_ministry_messages').select('id,congregation_id,created_by,message_type,title,body,active,publish_at,expires_at,created_at,media_path,pinned').gte('created_at',since).order('created_at',{ascending:false}).limit(100),
    a.from('bible_polls').select('id,congregation_id,created_by,prompt,options,active,closes_at,created_at,poll_type,results_visibility').gte('created_at',since).order('created_at',{ascending:false}).limit(100),
    ...
    a.from('bible_ministry_calendar').select('*').gte('starts_at',since).order('starts_at').limit(100),
    a.from('bible_member_recognitions').select('*').gte('created_at',since).order('created_at',{ascending:false}).limit(100),
    a.from('bible_media_library').select('id,congregation_id,created_by,media_type,title,youtube_url,featured,display_order,active,publish_at,created_at').gte('created_at',since).order('featured',{ascending:false}).order('display_order').limit(100),
    health(a)
  ]);
  for(const r of [c,p,as,m,po,se,cal,rec,med])if(r.error)throw r.error;
  const assignments=as.data||[],polls=po.data||[],sessions=se.data||[],media=med.data||[],aids=assignments.map((x:any)=>x.id),pids=polls.map((x:any)=>x.id),sids=sessions.map((x:any)=>x.id);
  const [prog,votes,parts]=await Promise.all([aids.length?a.from('bible_assignment_progress').select('assignment_id,user_id,status,completed_at,updated_at').in('assignment_id',aids):Promise.resolve({data:[],error:null}),pids.length?a.from('bible_poll_votes').select('poll_id,user_id,option_index').in('poll_id',pids):Promise.resolve({data:[],error:null}),sids.length?a.from('bible_session_participants').select('session_id,user_id').in('session_id',sids):Promise.resolve({data:[],error:null})]);for(const r of [prog,votes,parts])if(r.error)throw r.error;
  const ids=[...new Set([...(p.data||[]).map((x:any)=>x.user_id),...assignments.map((x:any)=>x.created_by),...(m.data||[]).map((x:any)=>x.created_by),...polls.map((x:any)=>x.created_by),...sessions.map((x:any)=>x.created_by),...media.map((x:any)=>x.created_by)].filter(Boolean))];let members:any[]=[];if(ids.length){const mr=await a.from('bible_congregation_members').select('congregation_id,user_id,display_name,role,avatar,active').in('user_id',ids);if(mr.error)throw mr.error;members=mr.data||[]}
