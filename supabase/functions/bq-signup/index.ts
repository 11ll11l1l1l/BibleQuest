...
const DEFAULT_CONGREGATION_NAME='ICAC';
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const email=cleanText(body.email,254).toLowerCase(),password=String(body.password||''),confirm=String(body.confirm_password||''),fullName=cleanText(body.full_name,120),preferredName=cleanText(body.preferred_name,40),avatar=cleanAvatar(body.avatar);
    ...
    // congregation later from Admin & Ministry.
    const defaultCongregation=await admin.from('bible_congregations').select('id,name').eq('name',DEFAULT_CONGREGATION_NAME).eq('active',true).order('created_at',{ascending:true}).limit(1).maybeSingle();
    if(defaultCongregation.error)throw defaultCongregation.error;
    if(!defaultCongregation.data)return json(req,{error:'BibleQuest registration is temporarily unavailable because the default ICAC congregation is not configured.'},503);
    const congregation=defaultCongregation.data;

    const {data:created,error:createError}=await admin.auth.admin.createUser({email,password,email_confirm:true,user_metadata:{full_name:fullName,preferred_name:preferredName,church_group:congregation.name,avatar,onboarding_complete:true,email_unverified:true}});
    ...
    const membership=await admin.from('bible_congregation_members').upsert({congregation_id:congregation.id,user_id:created.user.id,role:'member',display_name:preferredName,avatar,active:true,joined_at:new Date().toISOString()},{onConflict:'congregation_id,user_id'});
    if(membership.error){await admin.auth.admin.deleteUser(created.user.id).catch(()=>{});throw membership.error}
