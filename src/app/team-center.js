...
export function createTeamCenterService({api,session,congregation}){
  ...
  let teams=[],congregations=[],directory=[],contextUserId='',loadRequest=0;
  ...
  const clearContext=(userId='')=>{teams=[];congregations=[];directory=[];contextUserId=String(userId||'')};
  const contextCurrent=userId=>Boolean(userId)&&contextUserId===userId&&currentUserId()===userId;
  const snapshot=()=>{const state=sessionState(),visible=!contextUserId||contextUserId===currentUserId();return Object.freeze({authenticated:state.authenticated===true,remoteAvailable:state.remoteAvailable!==false,teams:visible?teams.slice():[],congregations:visible?congregations.slice():[],directory:visible?directory.slice():[]})};
  ...
  async function load(){
    ...
    const nextCongregations=memberships.map(row=>Object.freeze({id:row.congregationId,name:row.congregation.name,role:row.role,roleLabel:row.roleLabel,canManage:congregation.can(row.congregationId,'ministry')}));
    const congregationIds=nextCongregations.map(row=>row.id),allowedCongregations=new Set(congregationIds);
    if(!congregationIds.length){congregations=nextCongregations;teams=[];directory=[];return snapshot()}
    let result;
    try{result=await api.list(congregationIds)}catch(error){if(request!==loadRequest||currentUserId()!==userId)return snapshot();throw error}
    if(request!==loadRequest||currentUserId()!==userId)return snapshot();
    const rawTeams=Array.isArray(result?.teams)?result.teams:[],rawMembers=Array.isArray(result?.members)?result.members:[],rawDirectory=Array.isArray(result?.directory)?result.directory:[];
    ...
    const nextTeams=normalizedTeams.map(team=>{
      const ownCongregation=nextCongregations.find(row=>row.id===team.congregationId);if(!ownCongregation)throw teamError('Team Center received a team outside this account congregation scope.','BQ_TEAM_CENTER_SCOPE');
      ...
      const availableMembers=nextDirectory.filter(row=>row.congregationId===team.congregationId&&!memberIds.has(row.userId));
    ...
    if(request!==loadRequest||currentUserId()!==userId)return snapshot();
    congregations=nextCongregations;directory=nextDirectory;teams=nextTeams;return snapshot();
  }
  ...
  async function addMember(teamId,targetUserId){
    const userId=identity();if(!contextCurrent(userId))throw teamError('Reload Team Center after changing accounts.','BQ_TEAM_CENTER_CONTEXT_STALE');const team=requireManage(findTeam(teamId)),target=team.availableMembers.find(row=>row.userId===String(targetUserId||''));if(!target)throw teamError('Choose an active congregation member who is not already on this team.','BQ_TEAM_CENTER_TARGET');
  ...
  async function removeMember(teamId,targetUserId){
    const userId=identity();if(!contextCurrent(userId))throw teamError('Reload Team Center after changing accounts.','BQ_TEAM_CENTER_CONTEXT_STALE');const team=requireManage(findTeam(teamId)),member=team.members.find(row=>row.userId===String(targetUserId||''));if(!member)throw teamError('That account is not a member of this team.','BQ_TEAM_CENTER_TARGET');if(member.isCreator)throw teamError('The team creator stays a member while the team is active.','BQ_TEAM_CENTER_CREATOR_MEMBER');
    await api.remove(team.congregationId,team.id,member.userId);await load();return snapshot();
  }
  async function rename(teamId,name){
    const userId=identity();if(!contextCurrent(userId))throw teamError('Reload Team Center after changing accounts.','BQ_TEAM_CENTER_CONTEXT_STALE');const team=requireManage(findTeam(teamId)),title=cleanText(name,60);if(title.length<2)throw teamError('Enter a team name.','BQ_TEAM_CENTER_NAME');await api.rename(team.congregationId,team.id,title);await load();return snapshot();
  }
  async function archive(teamId){
    const userId=identity();if(!contextCurrent(userId))throw teamError('Reload Team Center after changing accounts.','BQ_TEAM_CENTER_CONTEXT_STALE');const team=findTeam(teamId);if(!team?.canArchive)throw teamError('Only the team creator or congregation admin can archive this team.','BQ_TEAM_CENTER_PERMISSION');congregation.assert(team.congregationId,'ministry');await api.archive(team.congregationId,team.id);await load();return snapshot();
  }
