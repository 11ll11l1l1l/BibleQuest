...
export function createCouplesCloudService({api,session}){
  ...
  async function load(){const userId=account(),result=await api.status();acceptPair(result?.pair||null,userId);if(pair?.status==='active')await refreshShared();return snapshot()}
  async function createPair(){const userId=account(),result=await api.create();acceptPair(result?.pair||null,userId);inviteCode=cleanText(result?.inviteCode||'',16).toUpperCase();if(pair?.status==='pending'&&!PAIR_CODE.test(inviteCode))throw cloudError('Couples cloud did not return a valid 8-character pair code.','BQ_COUPLES_CLOUD_MALFORMED');if(pair?.status==='active')await refreshShared();return snapshot()}
  ...
  async function refreshShared(){account();if(!pair||pair.status!=='active'){shared=[];return []}shared=(await api.listShared(pair.id)).map(row=>normalizeShared(row,pair.id)).sort((a,b)=>Date.parse(b.createdAt)-Date.parse(a.createdAt)||a.id.localeCompare(b.id));return shared.slice()}
  async function completeJourney(step,title,commitment=''){const userId=account();if(!pair||pair.status!=='active')throw cloudError('Link both Couple Journey accounts before saving shared progress.','BQ_COUPLES_CLOUD_PAIR_REQUIRED');const n=Number(step),label=cleanText(title,120),note=cleanText(commitment,MAX_COMMITMENT);if(!Number.isInteger(n)||n<1||n>7||!label)throw new Error('Choose a valid Couple Journey step.');const key=`${n}|${label}`,already=shared.some(item=>item.itemType==='journey'&&item.body===key),rows=[];if(!already)rows.push({pair_id:pair.id,author_id:userId,item_type:'journey',body:key});if(note)rows.push({pair_id:pair.id,author_id:userId,item_type:'commitment',body:note});if(rows.length)await api.addShared(rows);await refreshShared();return snapshot()}
  async function leave(){account();if(!pair?.id)throw cloudError('No Couple Journey pair is linked.','BQ_COUPLES_CLOUD_PAIR_REQUIRED');await api.leave(pair.id);pair=null;shared=[];inviteCode='';return snapshot()}
