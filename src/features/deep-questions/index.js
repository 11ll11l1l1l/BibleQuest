  ...
  return{
    ...
    mount(root){
      ...
      const renderActive=result=>{
        ...
        host.innerHTML=`<section class="bq-game-topline"><button type="button" class="bq-secondary-button" data-deep-library>All questions</button>${state.index>0?'<button type="button" class="bq-secondary-button" data-deep-reader="0">Open first passage</button>':''}</section><section class="bq-panel bq-deep-session" data-deep-session="${escapeHtml(item.id)}"><div class="bq-game-meta"><span>Deep Question</span><span>Step ${state.index+1} of ${state.totalSteps}</span></div><div class="bq-game-progress" aria-label="${percent}% complete"><i style="width:${percent}%"></i></div>${body}${DEEP_SOURCE}</section>`;
      };
