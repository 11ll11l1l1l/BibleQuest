...
const regionArt=key=>V4_ART.bibleWorld[key]||V4_ART.bibleWorld.map;

function worldArtwork(state,failed){
  const art=state.artwork||{},percent=Math.max(0,Math.min(100,Number(art.revealPercent)||0));
  if(failed)return `<section class="bq-world-art bq-world-art-fallback" data-world-art-fallback role="status"><div class="bq-world-art-fallback-icon" aria-hidden="true">${decorativeImg(V4_ART.bibleWorld.map,'bq-world-icon-art')}</div><div><b>Bible World artwork is unavailable.</b><p>${percent}% of the world is revealed from your learning evidence. The map and every Scripture route remain available below.</p></div></section>`;
