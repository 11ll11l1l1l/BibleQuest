export function morePage({pwaInstall,onCongregation,onBackup}){
  return {
    ...
    mount(root){
      ...
      congregation?.addEventListener('click',open);backupButton?.addEventListener('click',openBackup);installButton?.addEventListener('click',install);
      const unsubscribe=pwaInstall?.subscribe?.(render)||(()=>{});
      return()=>{congregation?.removeEventListener('click',open);backupButton?.removeEventListener('click',openBackup);installButton?.removeEventListener('click',install);unsubscribe()}
    }
  };
}
