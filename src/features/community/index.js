...
export function communityPage({bridge,onNavigate,onBack,onAccount}={}){
  return{title:'Community',html:'<section class="bq-community" data-community-view></section>',mount(root){
    ...
    const actions=state=>`<section class="bq-community-grid"><button type="button" data-community-route="congregation"><span>⛪</span><b>Membership & role</b><small>View or join a congregation</small></button><button type="button" data-community-route="leaderboards" ${state.status==='ready'&&state.congregations.length?'':'disabled'}><span>🏆</span><b>Leaderboards</b><small>Today · Week · All time</small></button><button type="button" data-community-route="journey-groups" ${state.status==='ready'?'':'disabled'}><span>👥</span><b>Journey Groups</b><small>Open your 2–6 person groups</small></button><button type="button" data-community-route="encouragements" ${state.status==='ready'&&state.groups.length?'':'disabled'}><span>💛</span><b>Encouragements</b><small>Send one of five safe presets</small></button></section>`;
    const render=state=>{if(disposed)return;const intro='<div class="bq-community-head"><div><p class="bq-eyebrow">COMMUNITY BRIDGE</p><h1>Grow together without exposing private study.</h1><p>One safe connection point for congregation access, rankings, Journey Groups, and preset Encouragements.</p></div><button type="button" class="bq-secondary-button" data-community-back>Back to More</button></div>';
      ...
      else view.innerHTML=`${intro}${actions(state)}${readyView(state)}<section class="bq-panel bq-community-boundary"><h2>What stays separate</h2><p>Private notes, answers, reflections, Couple Journey data, score submission, XP, presence, teams, assignments, notifications, and ministry controls do not cross this bridge. Leaderboards are opened as their own read-only owner.</p></section>`;bind()};
