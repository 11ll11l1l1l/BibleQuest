...
const COPY=Object.freeze({
  en:Object.freeze({
    ...
    complete:'Bible Quest complete',completeBody:'You have completed all 1,189 chapters from Genesis through Revelation.'
  }),
  tl:Object.freeze({
    ...
    complete:'Tapos ang Bible Quest',completeBody:'Natapos mo ang lahat ng 1,189 kabanata mula Genesis hanggang Pahayag.'
  }),
  ceb:Object.freeze({
    ...
    complete:'Nahuman ang Bible Quest',completeBody:'Nahuman nimo ang tanang 1,189 ka kapitulo gikan sa Genesis hangtod Pinadayag.'
  })
});
  ...
  return {
    ...
    mount(root){
      ...
      const render=()=>{
        ...
        const questTargets=state.today.map((item,index)=>`<li><b>${index+1}. ${esc(referenceLabel(item))}</b>${index===0?'<span> · next</span>':''}</li>`).join('');
        ...
        host.innerHTML=`<section class="bq-hero"><div><p class="bq-eyebrow">${esc(t.eyebrow)}</p><h1>${esc(t.title)}</h1><p>${esc(t.intro)}</p></div></section>
          ...
          ${state.complete?`<section class="bq-panel"><p class="bq-eyebrow">${esc(t.complete)}</p><h2>Genesis 1 → Revelation 22</h2><p>${esc(t.completeBody)}</p></section>`:`
          <section class="bq-panel" data-bible-quest-current>
            ...
            <h3>${esc(t.related)}</h3>
            <div class="bq-external-links"><button type="button" class="bq-secondary-button" data-bible-quest-continue>Open in BibleQuest Reader</button>${related.map(link=>`<a ${externalAttrs} href="${esc(link.href)}">${esc(link.label)}</a>`).join('')}</div>
          </section>
          <section class="bq-panel"><p class="bq-eyebrow">${esc(t.today)}</p><ol>${questTargets}</ol><div class="bq-daily-actions">${bibleQuest.allowedPaces.map(pace=>`<button type="button" class="${pace===state.pace?'bq-primary-button':'bq-secondary-button'}" data-bible-quest-pace="${pace}">${pace}/day</button>`).join('')}</div></section>`}
          <section class="bq-panel"><h2>Genesis → Revelation</h2><ul class="bq-home-assignment-list">${books}</ul></section>
