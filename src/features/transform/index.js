  ...
  return{
    ...
    mount(root){
      ...
      const renderBasic=state=>{
        ...
        const scriptureFlow=`<section class="bq-panel" data-transform-scripture-flow><p class="bq-eyebrow">SCRIPTURE → UNDERSTAND → REFLECT → APPLY → PRAY</p><h2>Move from the passage to a faithful next step</h2><p>This private journal uses the existing Transform storage owner. It does not grade faith or create a leader-visible response.</p><label class="bq-field"><span>1 · Scripture — passage or key verse</span><textarea rows="2" maxlength="500" data-transform-flow-scripture>${escapeHtml(reflection.scripture)}</textarea></label><label class="bq-field"><span>2 · Understand — what does it mean in context?</span><textarea rows="3" maxlength="1200" data-transform-flow-understand>${escapeHtml(reflection.understand)}</textarea></label><label class="bq-field"><span>3 · Reflect — what does this reveal or challenge?</span><textarea rows="3" maxlength="1200" data-transform-flow-reflect>${escapeHtml(reflection.reflect)}</textarea></label><label class="bq-field"><span>4 · Apply — one concrete next action</span><textarea rows="3" maxlength="1200" data-transform-flow-apply>${escapeHtml(reflection.apply)}</textarea></label><label class="bq-field"><span>5 · Pray — respond to God</span><textarea rows="3" maxlength="1200" data-transform-flow-pray>${escapeHtml(reflection.pray)}</textarea></label><div class="bq-transform-actions"><p class="bq-form-message" data-transform-flow-message aria-live="polite">${escapeHtml(message)}</p><button type="button" class="bq-primary-button" data-transform-flow-save>Save Scripture reflection</button></div></section>`;
