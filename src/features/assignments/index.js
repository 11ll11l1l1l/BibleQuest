...
function detailView(state){
  ...
  const start=progress.status==='assigned'?`<button type="button" class="bq-secondary-button" data-assignment-start="${esc(row.id)}">Start task</button>`:'';
  const completion=progress.status!=='completed'?`<form class="bq-account-form" data-assignment-complete="${esc(row.id)}"><label>Optional response to your leader<textarea name="submission" maxlength="4000" rows="4" placeholder="Share only what you intentionally want your leader to receive."></textarea></label><small>Your private Bible notes, journal, Transform answers and Couple Journey data are not attached.</small><button type="submit" class="bq-primary-button">Mark complete</button></form>`:'';
  return `<section class="bq-account-section" data-assignment-detail="${esc(row.id)}"><div class="bq-team-center-head"><div><p class="bq-eyebrow">${esc(labels[row.type]||row.type)} · ${esc(statusLabel(progress.status))}</p><h2>${esc(row.title)}</h2></div><button type="button" class="bq-secondary-button" data-assignment-close>Close task</button></div>${row.instructions?`<p>${esc(row.instructions)}</p>`:'<p>No additional instructions were provided.</p>'}${refs}<p><b>Due:</b> ${esc(formatDate(row.dueAt))} · <b>Completion points:</b> ${esc(String(row.points))}</p><div class="bq-account-actions">${start}</div>${completion}${response}${feedback}</section>`;
}
