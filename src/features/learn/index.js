...
export function learnPage({ onReader, onStudy, onDeepQuestions, onStoryJourney, onWisdomSituations, onAdaptiveLearning, onOpenReview, onBibleWorld, onExplorer, onPrivateNotes, onCloudNotes, translations = [], recallSource = null }) {
  ...
  return {
    title: 'Learn',
    html: `<section class="bq-panel"><p class="bq-eyebrow">LEARN</p><h1>Learn</h1><p>Read Scripture directly, follow a guided study, examine a deep question, walk through a Bible story, practice difficult Scripture-informed judgment, explore the biblical story map, review weak areas with spaced retrieval, or keep notes locally or with your signed-in account.</p></section>` +
      `<section class="bq-panel bq-learn-primary" data-learn-primary><button type="button" class="bq-learn-primary-button" data-open-reader><span class="bq-learn-primary-icon" aria-hidden="true">${iconSvg('bible', { size: 28 })}</span><span class="bq-learn-primary-text"><b>Bible Reader</b><span>English BSB + Tagalog ULB · search · verse tools</span></span></button></section>` +
      `<div class="bq-learn-group"><p class="bq-eyebrow">STUDY &amp; REFLECT</p><div class="bq-learning-grid">` +
      ...
      `<div class="bq-learn-group"><p class="bq-eyebrow">EXPLORE &amp; REVIEW</p><div class="bq-learning-grid">` +
        ...
        `<button type="button" class="bq-learning-card" data-open-explorer><b>Characters & Places</b><span>People · places · non-repeating clues · Scripture references</span></button>` +
        `<button type="button" class="bq-learning-card" data-open-adaptive-learning><b>Adaptive Learning</b><span>Due multiple-choice questions · weak areas · spaced Smart Review</span></button>` +
        `<button type="button" class="bq-learning-card" data-open-open-review><b>Open Smart Review</b><span>Answer from memory · reveal source answer · Got it / Review again</span></button>` +
      `</div></div>` +
      `<div class="bq-learn-group"><p class="bq-eyebrow">NOTES</p><div class="bq-learning-grid">` +
        `<button type="button" class="bq-learning-card" data-open-private-notes><b>Private Notes</b><span>Device-only · create · edit · delete · export</span></button>` +
        `<button type="button" class="bq-learning-card" data-open-cloud-notes><b>Cloud Notes</b><span>Account-based · Scripture-linked · cross-device</span></button>` +
      `</div></div>` +
      `<section class="bq-panel" data-doctrinal-policy><p class="bq-eyebrow">CONTENT SAFETY</p><h2>How scored Bible questions are handled</h2><p>Binary-scored questions must stay tied to an explicit Scripture reference and pass doctrinal-safety review. A disputed or universal doctrine claim is quarantined instead of being guessed into a right/wrong answer. Passage-sensitive material stays contextual, while Deep Questions and Wisdom remain interpretive or applied exercises rather than spiritual-quality scores.</p><p><small>${DOCTRINAL_SAFETY.authority}</small></p></section>${guide}`,
    mount(root) {
