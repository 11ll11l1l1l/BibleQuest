...
export function resolveTabFocus(options: TabFocusOptions): TabFocusResult {
  if (options.key !== 'Tab' || !validItemCount(options.itemCount)) {
    return Object.freeze({ handled: false, index: options.currentIndex });
  }

  const step = options.shiftKey === true ? -1 : 1;
  if (
    !Number.isInteger(options.currentIndex) ||
    options.currentIndex < 0 ||
    options.currentIndex >= options.itemCount
  ) {
    return Object.freeze({
      handled: true,
      index: step > 0 ? 0 : options.itemCount - 1,
    });
  }

  const candidate = options.currentIndex + step;
  if (candidate >= 0 && candidate < options.itemCount) {
    return Object.freeze({ handled: true, index: candidate });
  }
