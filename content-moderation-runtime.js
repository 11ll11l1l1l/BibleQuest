(() => {
  ...
  const STORAGE='biblequest_content_policy_v1';
  ...
  async function loadPolicy(congregationId,force=false){
    ...
    try{
      ...
      const request=c.from('bible_content_decisions').select('content_key,decision,content_type,origin,updated_at').eq('congregation_id',congregationId).limit(4000);
      const timeout=new Promise((_,reject)=>setTimeout(()=>reject(new Error('moderation policy timeout')),1400));
  ...
  function register(code,rows){for(const row of rows||[]){if(!row?.q)continue;const key=`question:${code}:${row.id}`;registry.set(normalize(row.q),{key,contentType:'question',ref:row.r||'',answer:row.a||'',source:`question-pack:${code}`,row})}}
  async function applyPack(response,code){
    ...
    let next=rows.filter(row=>!['exempt','remove'].includes(decisions.get(`question:${code}:${row.id}`)?.decision));
    ...
    if(includes.length){
      try{
        ...
        if(q.ok){const quarantined=await q.json();const have=new Set(next.map(r=>String(r.id)));for(const row of Array.isArray(quarantined)?quarantined:[]){const key=`question:${code}:${row.id}`;if(decisions.get(key)?.decision!=='include'||have.has(String(row.id)))continue;next.push({...row,safety:{...(row.safety||{}),originalAction:row.safety?.action||'quarantine',action:'allow',moderationOverride:'include'}});have.add(String(row.id))}}
      }catch(_err){}
    }
    register(code,next);
  ...
  async function applyCore(){
    ...
    const congregationId=activeCongregationId(),decisions=policyMap(await loadPolicy(congregationId));
    const next=[];for(const row of window.BQ_QUESTIONS){const key=`question:core:${row.id}`;registry.set(normalize(row.q),{key,contentType:'question',ref:row.ref||'',answer:row.a||row.answer||'',source:'core-question',row});if(!['exempt','remove'].includes(decisions.get(key)?.decision))next.push(row)}
    if(next.length!==window.BQ_QUESTIONS.length)window.BQ_QUESTIONS=next;
  }
  ...
  function decisionFor(key){return memory.rows.find(r=>r.content_key===key)?.decision||''}
  ...
  window.BQContentModeration={resolve,decisionFor,loadPolicy,refresh,activeCongregationId,genericKey,diagnostics:()=>({congregationId:memory.congregationId,decisions:memory.rows.length,registry:registry.size})};
})();
