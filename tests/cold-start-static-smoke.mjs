...
assert.ok(pos('progress-sync-policy.js')>pos('cold-start-hardening.js')&&pos('account.js')>pos('progress-sync-policy.js'),'conflict-safe progress policy must load between startup hardening and account');
...
for(const file of ['./cloud-config.js','./cold-start-hardening.js','./progress-sync-policy.js','./account.js','./guest-access-hardening.js','./linked-activities.js','./team-center.js'])assert.ok(sw.includes(`'${file}'`),`service worker should include ${file}`);
