...
assert.match(source, /async function requireSessionRevocation\(a:ReturnType<typeof db>,targetUserId:string\)/, 'sensitive actions must share a required session-revocation boundary');
assert.match(source, /a\.rpc\('bible_revoke_auth_sessions',\{target_user_id:targetUserId\}\)/, 'session revocation must use the service-role-only database RPC');
assert.match(source, /if\(r\.error\)throw new Error\(`Session revocation failed: \$\{r\.error\.message\}`\)/, 'required session revocation must fail closed on RPC errors');
assert.doesNotMatch(source, /\/auth\/v1\/admin\/users\/.*\/logout/, 'admin ops must not use a nonexistent admin logout-by-user-id HTTP route');
...
assert.match(suspend, /if\(action==='suspend_account'\)await requireSessionRevocation\(a,target\)/, 'suspend must require successful target session revocation');
assertBefore(suspend, 'await requireSessionRevocation(a,target)', "a.from('bible_app_access').update", 'suspend must revoke sessions before disabling the account');
...
assert.match(forceSignOut, /forceSignOutUser\(a,target\)/, 'force sign-out must revoke target sessions through the same RPC boundary');
assert.match(forceSignOut, /audit\(a,u\.id,target,'force_sign_out',\{\}\)/, 'force sign-out must be audited without sensitive detail');
assert.match(forceSignOut, /return json\(req,\{ok:true,revoked:true,revokedSessions\}\)/, 'force sign-out success must report explicit revocation and the non-sensitive deleted-session count');
...
assert.match(tempPassword, /await requireSessionRevocation\(a,target\)/, 'temporary password must require successful session revocation');
assertBefore(tempPassword, 'await requireSessionRevocation(a,target)', 'a.auth.admin.updateUserById(target,{password})', 'temporary password must revoke sessions before changing credentials');
...
assert.match(changeEmail, /await requireSessionRevocation\(a,target\)/, 'change_email must require successful global session revocation');
assertBefore(changeEmail, 'await requireSessionRevocation(a,target)', 'a.auth.admin.updateUserById(target,{email})', 'change_email must revoke sessions before changing the Auth identity');
