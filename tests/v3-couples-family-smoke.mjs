...
async function run(){
  ...
  await page.locator('[data-couples-go="dashboard"]').click();await page.locator('[data-couples-category="christ"]').click();const scriptureCardId=await page.locator('[data-couples-card-id]').getAttribute('data-couples-card-id');const expectedScriptureCard=COUPLES_CARDS.find(card=>card.id===scriptureCardId);assert(expectedScriptureCard,'Christ category did not open a known Couples card.');await page.locator('[data-couples-reader]').click();await page.waitForURL(/#\/reader$/);await page.locator('[data-reader-page]').waitFor();await page.waitForFunction(expected=>{const state=JSON.parse(localStorage.getItem('biblequest.v3.reader-state')||'{}');return state.translation==='bsb'&&state.book===expected.code&&Number(state.chapter)===expected.chapter},{code:expectedScriptureCard.code,chapter:expectedScriptureCard.chapter});const readerState=await page.evaluate(()=>JSON.parse(localStorage.getItem('biblequest.v3.reader-state')||'{}'));assert(readerState.translation==='bsb'&&readerState.book===expectedScriptureCard.code&&Number(readerState.chapter)===expectedScriptureCard.chapter,'Couples Scripture handoff did not open the selected card in the existing BSB Reader owner.');
