...
async function run(){
  ...
  await page.locator('[data-couples-go="dashboard"]').click();await page.locator('[data-couples-category="christ"]').click();await page.locator('[data-couples-reader]').click();await page.waitForURL(/#\/reader$/);await page.locator('[data-reader-page]').waitFor();const readerState=await page.evaluate(()=>JSON.parse(localStorage.getItem('biblequest.v3.reader-state')||'{}'));assert(readerState.translation==='bsb'&&['COL','PSA','PHP','EPH'].includes(readerState.book),'Couples Scripture handoff did not use the existing BSB Reader owner.');
