...
test('Detective, Recall, Timeline and round-result chrome use purpose-matched existing assets',async()=>{
  const [css,finalCss,games]=await Promise.all([
    read('src/ui/v4-custom-art.css'),read('src/ui/games-art-final-v4.css'),Promise.all([read('src/features/games/index.js'),read('src/features/games/views/launcher-memory.js'),read('src/features/games/views/same-room.js'),read('src/features/games/views/challenges.js')]).then(parts=>parts.join('\n'))
  ]);
...
test('remaining Games glyphs are semantic labels/fallback tokens, not falsely claimed artwork matches',async()=>{
  const [games,finalCss]=await Promise.all([read('src/features/games/index.js'),read('src/ui/games-art-final-v4.css')]);
