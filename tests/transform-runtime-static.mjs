...
assert.match(html,/<script src="client-diagnostics\.js"><\/script>/,'Transform must load the shared diagnostic classifier');
...
assert.match(html,/RUNTIME_URL='transformation-v2\.js'/,'Transform recovery must target the isolated runtime');
assert.match(html,/\?bq-recover=/,'Transform bootstrap must have a cache-busting clean-runtime recovery path');
...
assert.match(html,/showFailure\(typedError\('Standalone account gate did not load\.'/,'Transform must visibly recover when the standalone account gate did not load');
assert.match(html,/showFailure\(typedError\('Transform account verification timed out\.'/,'Transform must visibly recover when account verification stalls');
assert.match(html,/BQ-NET-001/,'Transform fallback must distinguish an offline connection');
assert.match(html,/BQ-MOD-001/,'Transform fallback must distinguish an app-module failure');
