...
async function run(){
  ...
  assert(result.synced.row.includes('Read John 1 again')&&result.synced.message,'Realtime assignment signal did not refresh server truth.');assert(result.leader.text.includes('Recipient responses remain read-only')&&!result.leader.hasStart&&!result.leader.hasComplete&&result.leader.hasPublisher,'Ministry-role assignment recipient path must remain read-only while #75 publishing is available.');assert(result.cleaned===2,'Each mounted Assignments realtime channel must be cleaned up exactly once.');assert(result.signedOut==='signed-out','Signed-out Assignments did not fail closed.');
