...
try {
  ...
  await page.evaluate(async () => {
    ...
    const row = { id: 'recording-stable-123', title: 'Runtime Worship Title', description: 'Runtime description remains source data.', featured: true };
    let state = { status: 'ready', rows: [row], selectedId: null };
    const recordings = { load: async () => state, getState: () => state, select: (id, frameHost) => { state = { ...state, selectedId: id }; frameHost.textContent = 'runtime-player-host'; }, addVideo: async () => state, leave: () => {} };
  ...
  const geometry = await page.evaluate(() => ({ innerWidth: window.innerWidth, scrollWidth: document.documentElement.scrollWidth, minButtonHeight: Math.min(...[...document.querySelectorAll('[data-video-select], [data-video-curator-toggle], [data-recordings-home], [data-video-add-form] button')].map(node => node.getBoundingClientRect().height).filter(Boolean)) }));
