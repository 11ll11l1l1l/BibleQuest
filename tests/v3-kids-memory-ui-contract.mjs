...
const memoryView=fs.readFileSync('src/features/games/views/launcher-memory.js','utf8');
const uiSurface=ui+memoryView;
const css=fs.readFileSync('src/ui/games.css','utf8');

for(const token of ['data-memory-open','data-memory-replay','data-memory-index','data-memory-grid','data-memory-meadow','data-memory-complete','aria-live="polite"'])assert(uiSurface.includes(token),`Memory Meadow UI token missing: ${token}`);
assert(ui.includes('memoryWidth()'),'Memory Meadow must choose pair count from the current rendered width.');
assert(uiSurface.includes('state.locked||card.open||card.done'),'Resolved/open cards and locked pairs must not accept duplicate input.');
