...
const migration = await readFile(new URL('../supabase/migrations/20260913194500_push_subscriptions.sql', import.meta.url), 'utf8');
const compact = migration.replace(/\s+/g, ' ').toLowerCase();

const mustMatch = (pattern, message) => assert.match(compact, pattern, message);

test('push subscription table stores only current-architecture delivery material', () => {
  ...
  assert.doesNotMatch(compact, /vapid[_ -]?private|private[_ -]?key/, 'VAPID private material must never be stored in this browser-facing table');
});

test('one physical push endpoint cannot remain attached to multiple accounts', () => {
...
test('RLS limits subscription reads and writes to the owning account', () => {
...
test('subscription fields have bounded sizes', () => {
