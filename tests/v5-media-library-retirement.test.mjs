...
test('media route remains an alias of the verified Recordings page', async()=>{
  ...
  assert.match(bootstrap, /recordings:\(\)=>recordingsPage\(\{recordings,onHome:\(\)=>router\.navigate\('home'\),onAccount:\(\)=>router\.navigate\('account'\)\}\),media:\(\)=>recordingsPage\(\{recordings,onHome:\(\)=>router\.navigate\('home'\),onAccount:\(\)=>router\.navigate\('account'\)\}\)/);
  assert.match(bootstrap, /onMedia:\(\)=>router\.navigate\('media'\)/);
