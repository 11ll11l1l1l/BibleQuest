...
assert.match(notification,/notification-center-icon[^>]+aria-hidden="true"/,'notification icon wrapper remains decorative');
assert.match(notification,/alt=""/,'notification artwork stays decorative so localized title/body remain the accessible content');
assert.match(notification,/localization\.getLocale\(\)/,'notification artwork change must preserve the integrated localization owner');
...
assert.match(encouragement,/aria-hidden="true"/,'encouragement decorative renderer stays hidden from assistive technology');
