...
test('delete audit detail remains privacy-safe', () => {
  const block = deleteBlock();
  assert.equal(/password|token|secret|service_role/i.test(block.slice(block.indexOf("auditRequired"), block.indexOf("return json(req,{ok:true,deleted:true})"))), false,
    'delete audit path must not introduce credential-bearing detail');
