...
assert.deepEqual(state.memberTools.map(tool=>tool.id),['assignments','journey-groups','live-room']);
...
assert.equal(state.memberTools.find(tool=>tool.id==='live-room').available,false,'Live Room must remain deferred in #76.');
assert.equal(state.ministryTools.find(tool=>tool.id==='leader-dashboard').available,false,'Leader Dashboard must not be silently implemented in #76.');
...
assert.equal(state.memberTools.filter(tool=>tool.available).length,2);
