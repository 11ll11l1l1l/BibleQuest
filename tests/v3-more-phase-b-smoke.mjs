...
try{
  ...
  assert(await icons.count()===17,`Expected 17 More feature icons, found ${await icons.count()}.`);
  const hrefs=await page.locator('.bq-more-icon use').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('href')));
  assert(hrefs.length===17,'Every More icon must contain one external sprite reference.');
  assert(hrefs.every(href=>href?.startsWith('assets/more-feature-icons.svg#')),'More icon reference escaped the committed sprite.');
  assert(new Set(hrefs).size===17,'More feature cards must use distinct semantic sprite symbols.');
