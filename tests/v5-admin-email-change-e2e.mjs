...
async function signIn(email, password) {
  const { response, body } = await jsonFetch(`${base}/auth/v1/token?grant_type=password`, {
...
async function invoke(jwt, payload) {
  return jsonFetch(`${base}/functions/v1/bq-admin-ops`, {
    method: 'POST',
    headers: { ...anonHeaders, Authorization: `Bearer ${jwt}` },
    body: JSON.stringify(payload),
  });
}

async function serviceGetUser(id) {
  return jsonFetch(`${base}/auth/v1/admin/users/${id}`, { headers: serviceHeaders });
}

async function serviceUpdateUser(id, attributes) {
  return jsonFetch(`${base}/auth/v1/admin/users/${id}`, {
    method: 'PUT', headers: serviceHeaders, body: JSON.stringify(attributes),
  });
}

async function latestAudit(targetUserId) {
  const params = new URLSearchParams({
    select: 'actor_id,target_user_id,action,detail,created_at',
    target_user_id: `eq.${targetUserId}`,
    action: 'eq.change_email',
...
async function refresh(refreshToken) {
  return jsonFetch(`${base}/auth/v1/token?grant_type=refresh_token`, {
...
function assertAuditSafe(detail) {
  ...
  for (const sensitiveValue of [env.BQ_V5_TARGET_EMAIL, env.BQ_V5_TARGET_NEW_EMAIL, env.BQ_V5_TARGET_PASSWORD]) {
...
try {
  ...
  console.log('PASS: controlled non-production owner email-change E2E, admin denial, owner-self denial, Auth mutation, session revocation, and privacy-safe audit verified.');
} finally {
  if (targetId) {
