...
async function run(){
  ...
  assert(await page.locator('[data-game-launch]').count()===5,'Games launcher must expose five modes through Character Detective.');
  ...
  await page.locator('[data-game-launch="character-detective"]').click();await page.locator('[data-detective="d1"]').waitFor();
  assert(await page.locator('[data-detective-feedback]').count()===0,'Detective must not reveal feedback before a guess.');
  await page.locator('[data-detective-submit]').click();assert(await page.locator('[data-detective-feedback]').count()===0,'Empty Detective answer must be blocked by native validation.');
  ...
  assert((await page.locator('[data-game-last="character-detective"]').textContent())?.includes('0/1'),'Character Detective last result did not persist across reload.');
  await page.locator('[data-game-launch="per-book-recall"]').click();await page.locator('[data-recall-library]').waitFor();await page.locator('[data-recall-search]').fill('Ruth');await page.locator('[data-recall-book="RUT"]').waitFor();const summary=await page.locator('[data-recall-summary="RUT"]').textContent();assert(summary?.includes('1 review'),'Per-book Recall review queue did not persist across reload.');assert(summary?.includes('10 studied'),'Per-book Recall study stats did not persist across reload.');assert(summary?.includes('last 9/10'),'Per-book Recall last result did not persist across reload.');
  ...
  await page.locator('[data-game-launcher]').click();await page.locator('[data-game-launch="character-detective"]').waitFor();
