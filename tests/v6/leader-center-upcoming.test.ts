...
test('Calendar shared agenda never loads or exposes personal Calendar data', async () => {
  let personalCalls = 0;
  const session = { getState: () => ({ authenticated: true, user: { id: 'leader-a' } }) };
  const privateStorage = { read: () => null, write: () => undefined };
  const active = {
...
test('bootstrap wires Calendar before Leader Center and delegates Calendar navigation', () => {
...
test('Leader Center upcoming copy is present in EN, TL and Cebuano', () => {
  const keys = [
