...
describe('same-room game views',()=>{
  it('keeps setup player-count actions and launcher escape',()=>{
    ...
    for(const count of [2,3,4,5,6]) assert.match(html,new RegExp(`data-same-room-start="${count}"`));
  ...
  it('renders deterministic active-player, scoreboard and answer hooks',()=>{
    ...
    assert.match(html,/data-same-room-question="q1"/);
    assert.match(html,/Turn: <b>Player &lt;1&gt;<\/b>/);
    assert.match(html,/data-same-room-player="p1"/);
    assert.match(html,/data-same-room-answer="0"/);
    assert.doesNotMatch(html,/data-same-room-next/);
    assert.match(html,/<small data-source>source<\/small>/);
  });
  ...
  it('locks choices and exposes feedback only after an answer',()=>{
    ...
    assert.match(html,/data-same-room-feedback/);
    assert.match(html,/Review &lt;this&gt;/);
    assert.match(html,/See final scoreboard/);
    assert.match(html,/data-same-room-answer="1" disabled/);
  });
  ...
  it('renders winner and tie results without changing local score identity',()=>{
    ...
    assert.match(winner,/<h1>Winner: One<\/h1>/);
    assert.match(winner,/data-same-room-open/);
    const tie=renderSameRoomCompleteView({escapeHtml,state:{players:[{id:'p1',name:'One',score:2},{id:'p2',name:'Two',score:2}]}});
    assert.match(tie,/<h1>Tie: One &amp; Two<\/h1>/);
  });
});
