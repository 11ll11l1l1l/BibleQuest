...
function request(id = 'mutation-1', congregationId = 'congregation-a') {
  return {
    id,
    domain: 'example-progress',
    operation: 'upsert',
...
test('fresh service instance recovers durable due work only for the active account and congregation', async () => {
  ...
  const recovered = await restarted.recover({ accountId: 'user-1', congregationId: 'congregation-a' });
...
test('retry persists incremented attempt and due time across a fresh service instance', async () => {
  ...
  const retried = await service.retry(queued);
  ...
  const early = await new OfflineOutboxService(storage, [safe], () => clock.now)
    .recover({ accountId: 'user-1', congregationId: 'congregation-a' });
...
test('policy revocation blocks retry persistence and completion removes successful work', async () => {
  const storage = new MemoryPersistence();
  ...
  await assert.rejects(() => revoked.retry(queued), /not explicitly allowlisted/i);
  assert.equal(storage.records.get('mutation-1')?.attempt, 0);

  await service.complete('mutation-1');
