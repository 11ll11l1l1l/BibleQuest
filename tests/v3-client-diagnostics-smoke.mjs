...
const bootstrapSource=fs.readFileSync(new URL('../src/app/bootstrap.js',import.meta.url),'utf8');
assert(bootstrapSource.includes('contentState:offlineShell.getState'),'Live bootstrap does not bind diagnostics to the offline-shell content state.');
async function run(){
  ...
  await page.evaluate(async()=>{
    ...
    const diagnostics=createClientDiagnosticsService({
      probe:async()=>({reachable,status:reachable?200:0,reason:reachable?'ok':'http'}),
      online:()=>true,
      clock:()=>now,
      serviceWorkerState:()=>({supported:true,controlled:true,state:'activated'}),
      contentState:()=>({status:'ready',ready:true})
    });
    const diagnosticIdentity=(await diagnostics.classify(new Error('ignored'),{kind:'unknown',route:'diagnostics'})).build;
    const runtimeState=await diagnostics.runtimeState({forceProbe:true});
    ...
    window.__bqDiagnosticsHarness={build:diagnosticIdentity,runtime:runtimeState,navigate:r=>router.navigate(r),unreachable:()=>{reachable=false;now+=6000},destroy:()=>{root.remove();delete window.__bqDiagnosticsHarness}};router.navigate('broken');
  });
  const scope=page.locator('[data-diagnostics-test-root]');
  const build=await page.evaluate(()=>window.__bqDiagnosticsHarness.build);
  assert(typeof build?.sha==='string'&&build.sha.length>0,'Runtime diagnostics did not expose a build SHA.');
  assert(build.development===(build.sha==='development'),'Runtime diagnostics build development flag does not match its SHA.');
  const runtime=await page.evaluate(()=>window.__bqDiagnosticsHarness.runtime);
  assert(runtime.connectivity.browserOnline===true&&runtime.connectivity.serverReachable===true&&runtime.connectivity.status===200&&runtime.connectivity.reason==='ok','Browser diagnostics did not expose safe connectivity state.');
  assert(runtime.serviceWorker.supported===true&&runtime.serviceWorker.controlled===true&&runtime.serviceWorker.state==='activated','Browser diagnostics did not expose safe service-worker state.');
  assert(runtime.content.status==='ready'&&runtime.content.ready===true,'Browser diagnostics did not expose safe offline-content readiness.');
  assert(typeof runtime.at==='string'&&runtime.build.sha===build.sha,'Browser runtime diagnostics did not preserve timestamp/build identity.');
