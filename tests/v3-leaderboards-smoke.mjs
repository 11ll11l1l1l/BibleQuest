...
async function run(){
  ...
  const result=await page.evaluate(async()=>{
    ...
    let sessionState={authenticated:true,remoteAvailable:true,user:{id:'u1'}},calls=[],activeId='c1';
    const session={getState:()=>sessionState};
    const memberships=[
      {congregationId:'c1',roleLabel:'Member',congregation:{name:'Test Church',timezone:'Asia/Tokyo'}},
      {congregationId:'c2',roleLabel:'Member',congregation:{name:'Second Church',timezone:'Asia/Tokyo'}}
    ];
    const congregation={load:async()=>memberships,getActive:()=>memberships.find(row=>row.congregationId===activeId)||null,assert:()=>true};
    const api={load:async(id,since)=>{calls.push({id,since});return{directory:[{congregation_id:id,user_id:'u1',display_name:id==='c1'?'Amy':'Cara',active:true},{congregation_id:id,user_id:'u2',display_name:'Ben',active:true}],scores:[{user_id:'u1',category:'knowledge',points:8},{user_id:'u2',category:'reading',points:4}]}}};
    ...
    const afterTenantSwitch={period:leaderboards.snapshot().period,congregationId:leaderboards.snapshot().congregationId,title:host.querySelector('h1')?.textContent||'',rows:[...host.querySelectorAll('[data-leaderboard-user]')].map(row=>row.textContent.trim())};
    ...
    return{initial,knowledge,afterTenantSwitch,authCode,calls};
  });
  ...
  assert(result.afterTenantSwitch.period==='today','Today period did not apply.');
  assert(result.afterTenantSwitch.congregationId==='c2','Leaderboards filter reload did not follow the newly active congregation.');
  assert(result.afterTenantSwitch.title==='Second Church','Leaderboards did not refresh the visible congregation title after active-context switch.');
  assert(result.afterTenantSwitch.rows[0].includes('Cara'),'Leaderboards retained stale congregation member rows after active-context switch.');
