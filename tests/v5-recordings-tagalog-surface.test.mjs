...
test('migrated Videos renderer no longer embeds representative English UI chrome', () => {
  for (const leak of ['>Featured<','>Add a video<','>Back home<','<h1>Worship and Bible study videos</h1>','<h2>No videos yet</h2>','<h2>Choose a video</h2>','>Add video<']) assert.equal(source.includes(leak), false, `Videos source still embeds migrated English UI text: ${leak}`);
});
