...
const sameRoomEventsBefore=events.length;
const earlyFinish=make();let together=earlyFinish.startSameRoom(2);const firstTogetherAnswer=together.question.answer;together=earlyFinish.answerSameRoom(firstTogetherAnswer);assert.equal(together.players[0].score,1);const finishedTogether=earlyFinish.finishSameRoom();assert.equal(finishedTogether.phase,'same-room-complete');assert.equal(finishedTogether.index,finishedTogether.total,'Early Play Together finish must complete the shared V6 session.');assert.equal(finishedTogether.question,null);assert.equal(finishedTogether.locked,false);assert.deepEqual(finishedTogether.players.map(player=>player.score),[1,0]);assert.equal(events.length,sameRoomEventsBefore,'Play Together must remain local-only and never write profile Progress events.');

const teardown=make();
const resumable=teardown.start('quick-recall');teardown.answer(resumable.question.answer);const preserved=teardown.leave();assert.equal(preserved.phase,'question','Route teardown must preserve resumable solo rounds.');assert.equal(preserved.roundId,resumable.roundId);assert.equal(preserved.locked,true);
teardown.showLauncher();teardown.startSameRoom(2);teardown.answerSameRoom(0);assert.equal(teardown.getSameRoomState().players.length,2);teardown.leave();assert.equal(teardown.getSameRoomState().phase,'same-room-setup','Route teardown must abandon local Play Together state.');assert.equal(teardown.getSameRoomState().players.length,0);
teardown.start('character-detective');assert.equal(teardown.leave().phase,'launcher','Route teardown must abandon Detective transient state.');
teardown.start('timeline-challenge');assert.equal(teardown.leave().phase,'launcher','Route teardown must abandon Timeline transient state.');
await teardown.openRecallLibrary();assert.equal(teardown.leave().phase,'launcher','Route teardown must abandon Recall transient state.');
