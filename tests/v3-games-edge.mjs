...
for(const contract of['progress.record','storage.read','storage.write','recall.loadManifest','recall.loadBook','answerDetective','replayDetective'])assert.ok(serviceSource.includes(contract));
...
const games=make();assert.equal(games.getState().phase,'launcher');assert.deepEqual(games.modes.map(mode=>mode.id),['quick-recall','context-challenge','mixed-quest','per-book-recall','character-detective']);
...
const detectiveDuplicate=games.answerDetective('David');assert.equal(detectiveDuplicate.duplicate,true);assert.equal(events.length,detectiveEventsBefore+2,'Duplicate detective submission must not award XP twice.');
assert.equal(games.lastResult('character-detective').score,1);assert.equal(games.lastResult('character-detective').gained,12);
detective=games.replayDetective();assert.equal(detective.detectiveItem.id,'d2','Another detective must advance to another clue set.');checked=games.answerDetective('Moses');assert.equal(checked.correct,false);assert.equal(checked.score,0);assert.equal(checked.gained,3);assert.equal(games.lastResult('character-detective').gained,3);
games.showLauncher();assert.throws(()=>games.answerDetective('Joseph'),/Start Character Detective/);
