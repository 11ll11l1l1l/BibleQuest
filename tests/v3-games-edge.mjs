...
assert.equal(/\b(document|window|localStorage|sessionStorage|createClient)\b/.test(gameServiceSource),false,'Game Launcher service must remain DOM/storage/backend-implementation independent.');
const gameUiSource=fs.readFileSync(path.resolve('src/features/games/index.js'),'utf8');
assert.equal(/progress\.record|localStorage|sessionStorage|createClient|window\.BQ/.test(gameUiSource),false,'Games UI must not bypass its owner boundaries.');

assert.deepEqual(GAME_MODES.map(mode=>mode.id),['quick-recall','context-challenge']);
...
const games=createGameLauncherService({progress,roundIdFactory:(mode,sequence)=>`${mode}-round-${sequence}`});
