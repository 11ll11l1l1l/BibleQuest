...
rows.unshift({id:8,group_id:'g1',sender_id:'former-member',recipient_id:null,kind:'heart',created_at:'2026-09-08T08:00:00.000Z'});state=await service.load();assert(state.items.some(item=>item.senderId==='former-member'&&item.kind==='heart'),'Historical encouragement from a former member must remain readable.');
