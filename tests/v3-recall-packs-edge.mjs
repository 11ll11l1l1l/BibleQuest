...
assert.equal(ruth.items[0].contextNote,'','Plain textual Recall must not invent a context notice.');
...
assert.equal(contextual.contextNote,contextual.safety.contextNote,'Recall presentation context must come from the trusted current policy review.');
assert.match(contextual.contextNote,/passage|context|baptism/i);
assert.equal(contextual.answer,'The named believer.','BibleQuest context must not be merged into the imported reference answer.');
