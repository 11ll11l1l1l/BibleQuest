...
if(!failures.length){
  ...
  for(const item of['createOfflineShellService',"register('offline-shell-sw.js',{scope:'./'})","getEntriesByType?.('resource')",'BIBLEQUEST_WARM_SHELL','MessageChannelCtor','disposed','timeoutMs=15000'])if(!owner.includes(item))fail(`Offline Shell owner missing contract ${item}.`);
