...
if(!failures.length){
  ...
  for(const item of['createOfflineShellService',"register('offline-shell-sw.js',{scope:'./',updateViaCache:'none'})","getEntriesByType?.('resource')",'BIBLEQUEST_WARM_SHELL','MessageChannelCtor','disposed','timeoutMs=15000'])if(!owner.includes(item))fail(`Offline Shell owner missing contract ${item}.`);
  for(const item of["CACHE_PREFIX='biblequest-v3-offline-shell-'","addEventListener('install'","addEventListener('activate'","addEventListener('message'","addEventListener('fetch'",'skipWaiting','clients.claim','bq-net-probe','request.destination','request.mode===\'navigate\'','cache.match','WARM_CONCURRENCY=8','function staticImportUrls(source,baseUrl)','while(pending.length)','Promise.all(batch.map'])if(!worker.includes(item))fail(`Offline worker missing contract ${item}.`);
