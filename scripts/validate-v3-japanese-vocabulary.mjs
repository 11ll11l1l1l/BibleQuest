...
for(const contract of ["import { japaneseVocabularyBlock, japaneseVocabularyControl } from './vocabulary.js'",'vocabulary = null','furigana = null',"state.translation === 'jko'",'vocabulary.notesFor(peek.text)','vocabulary.setEnabled']) if(!readerUi.includes(contract)) fail(`Reader presentation missing vocabulary integration contract: ${contract}`);
...
if(!bootstrap.includes('const vocabulary=createJapaneseVocabularyService({storage})')||!bootstrap.includes('readerPage({reader,vocabulary,furigana,offlinePackages:offlineScripturePackages})')) fail('Bootstrap must compose vocabulary through Storage and inject it into Reader alongside the managed offline package boundary.');
