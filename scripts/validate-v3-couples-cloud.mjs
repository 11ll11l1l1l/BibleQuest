...
if(!failures.length){
  ...
  for(const item of['createCouplesCloudService({api,session})','BQ_COUPLES_CLOUD_AUTH_REQUIRED','BQ_COUPLES_CLOUD_REMOTE_DISABLED','BQ_COUPLES_CLOUD_PERMISSION','PAIR_CODE','completeJourney','refreshShared'])if(!owner.includes(item))fail(`Couples cloud owner missing contract: ${item}`);
  for(const forbidden of['localStorage','sessionStorage','createClient','@supabase','functions.invoke',".from('",'window.','document.','BQCommunity'])if(owner.includes(forbidden))fail(`Couples cloud owner bypasses central ownership: ${forbidden}`);
  for(const forbidden of['localStorage','sessionStorage','createClient','@supabase','functions.invoke',".from('",'BQCommunity'])if(ui.includes(forbidden))fail(`Couples cloud UI bypasses its owner/API boundaries: ${forbidden}`);
