...
if(!failures.length){
  ...
  for(const item of["functions.invoke('bq-couple'","from('bible_couple_shared')",'Share an 8-character code','Private Notes','Transformation results'])if(!legacy.includes(item))fail(`Legacy Couples cloud recovery evidence missing: ${item}`);
  ...
  if(/localStorage|sessionStorage|createClient|supabase|functions\.invoke|\.from\(|fetch\s*\(|window\.|document\.|BQCommunity|progress\./.test(owner))fail('Couples cloud owner bypasses central API/session ownership or legacy score globals.');
  if(/localStorage|sessionStorage|createClient|supabase|functions\.invoke|\.from\(|fetch\s*\(|BQCommunity|progress\./.test(ui))fail('Couples cloud UI bypasses its owner/API boundaries.');
