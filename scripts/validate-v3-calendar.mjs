...
if(!failures.length){
  ...
  for(const token of['normalizeEvent','fromAssignmentDue','buildAgenda'])if(!engine.includes(token))fail(`Calendar engine missing contract token: ${token}`);
  ...
  for(const token of['session.getState','privateStorage.read','privateStorage.write','api.calendar','assignments','BQ_CALENDAR_INPUT'])if(!service.includes(token))fail(`Calendar service missing lifecycle contract: ${token}`);
  ...
  for(const token of['data-calendar-add','data-calendar-back','data-calendar-remove','calendar.load','calendar.addEvent','calendar.removeEvent'])if(!ui.includes(token))fail(`Calendar UI missing presentation contract: ${token}`);
  ...
  for(const token of['calendar','bible_calendar_events'])if(!api.includes(token))fail(`API boundary missing Calendar composition: ${token}`);
  ...
  for(const token of['personal reminders','assignment due-date aggregation','Explicitly deferred','Congregation-shared','own-row RLS'])if(!contract.includes(token))fail(`Calendar contract missing recorded scope boundary: ${token}`);
