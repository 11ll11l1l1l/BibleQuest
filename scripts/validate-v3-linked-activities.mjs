...
const required=['LINKED_ACTIVITIES_V3.md','FEATURE_INVENTORY_V3.md','src/app/linked-activities.js','src/app/assignments.js','src/features/assignments/index.js','supabase/functions/bq-assignment/index.ts','tests/v3-linked-activities-edge.mjs','tests/v3-linked-activities-smoke.mjs','.github/workflows/v3-regression.yml'];
for(const file of required)if(!fs.existsSync(file))fail(`Missing #79 Linked Activities file: ${file}`);
if(!failures.length){
  const contract=read('LINKED_ACTIVITIES_V3.md'),service=read('src/app/linked-activities.js'),ui=read('src/features/assignments/index.js'),assignment=read('src/app/assignments.js'),server=read('supabase/functions/bq-assignment/index.ts'),inventory=read('FEATURE_INVENTORY_V3.md'),workflow=read('.github/workflows/v3-regression.yml');
  ...
  for(const item of["import {createLinkedActivitiesService} from '../../app/linked-activities.js'",'data-assignment-linked','data-linked-completion-handoff','linkedActivities.launch','linkedActivities.complete','location.hash=`#/${result.route}`'])if(!ui.includes(item))fail(`Assignments UI missing #79 handoff: ${item}`);
