...
const required=[
  'index.html','src/app/bootstrap.js','src/app/router.js','src/app/store.js','src/app/session.js','src/app/account.js','src/app/reader.js','src/app/daily-mission.js','src/app/transform.js','src/app/audio.js','src/app/recordings.js',
  'src/core/storage.js','src/core/api.js','src/core/bible.js','src/core/progress.js','src/engines/lesson.js','src/engines/transform.js',
  'src/features/transform/content.js','src/features/transform/index.js','src/features/recordings/index.js',
  'src/ui/shell.js','src/ui/app.css','src/ui/reader.css','src/ui/progress.css','src/ui/daily-mission.css','src/ui/transform.css','src/ui/recordings.css',
...
for(const style of['src/ui/app.css','src/ui/reader.css','src/ui/progress.css','src/ui/daily-mission.css','src/ui/transform.css','src/ui/recordings.css'])if(!html.includes(style))fail(`index.html must load ${style}.`);
...
for(const file of jsFiles){
  ...
  if(rel!=='src/app/audio.js'&&/(document\.createElement\(['"]iframe['"]\)|youtube-nocookie\.com\/embed\/)/.test(text))fail(`Embedded recording-player ownership leaked outside Audio owner: ${rel}`);
  if(rel!=='src/app/audio.js'&&/data-bq-audio-player/.test(text))fail(`Audio player instance marker leaked outside Audio owner: ${rel}`);
  if(/MutationObserver/.test(text)&&rel.includes('record'))fail(`MutationObserver is forbidden in v3 recordings runtime: ${rel}`);
}

const onlyOwner=(pattern,expected,label)=>{
  const owners=jsFiles.filter(file=>pattern.test(fs.readFileSync(file,'utf8')));
  if(owners.length!==1||path.relative(root,owners[0]).replaceAll('\\','/')!==expected)fail(`Exactly one ${label} owner is required: ${expected}`);
};
...
if(!api.includes('Live Recordings took too long to load'))fail('Live Recordings cloud request must remain bounded.');
...
const transformEngine=read('src/engines/transform.js');
if(/window\.|document\.|localStorage|sessionStorage|progress\.record|createClient/.test(transformEngine))fail('Transform engine must remain DOM/router/storage-implementation/API/progress independent.');
for(const name of['calculateSpiritual','calculatePersonality','calculateBias']){
  const owners=jsFiles.filter(file=>new RegExp(`function\\s+${name}\\s*\\(`).test(fs.readFileSync(file,'utf8')));
  if(owners.length!==1||path.relative(root,owners[0]).replaceAll('\\','/')!=='src/engines/transform.js')fail(`Transform derived-result function ${name} must be defined only by src/engines/transform.js.`);
}
const transformService=read('src/app/transform.js');
for(const contract of['transform:spiritual:v1:complete','progress.record','engine.calculateSpiritual'])if(!transformService.includes(contract))fail(`Transform orchestration missing contract ${contract}.`);
if(/localStorage|sessionStorage|\.xp\s*=|\.streak\s*=/.test(transformService))fail('Transform orchestration bypasses storage/progress ownership.');
const transformUi=read('src/features/transform/index.js');
...
if(/document\.|window\.|localStorage|sessionStorage|createClient/.test(recordings))fail('Recordings lifecycle owner must remain DOM/storage/backend-implementation independent.');
const recordingsUi=read('src/features/recordings/index.js');
if(/createElement\(['"]iframe|youtube-nocookie|createClient|bible_media_library|localStorage|sessionStorage/.test(recordingsUi))fail('Recordings UI bypasses Audio/API/storage ownership.');

const inventory=read('FEATURE_INVENTORY_V3.md'),allowed=new Set(['Not started','Implemented','Verified','Regression-tested']);
...
for(const owner of['src/core/bible.js','src/app/reader.js','src/core/progress.js','src/engines/lesson.js','src/app/daily-mission.js','src/engines/transform.js','src/app/transform.js','src/app/audio.js','src/app/recordings.js'])if(!architecture.includes(owner))fail(`Architecture document must name active owner ${owner}.`);
const status=read('DEVELOPMENT_STATUS_V3.md');
if(!status.includes('Defect / root-cause ledger')||!status.includes('Next major milestone'))fail('Development status must retain defect ledger and next-work queue.');
...
console.log(`Checked ${jsFiles.length} v3 JavaScript modules and ${rows.length} inventory rows.`);
