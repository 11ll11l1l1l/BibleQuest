...
const required=[
  'index.html','src/app/bootstrap.js','src/app/router.js','src/app/store.js','src/app/session.js','src/app/account.js','src/app/reader.js','src/app/study.js','src/app/deep-questions.js','src/app/story-journey.js','src/app/wisdom-situations.js','src/app/adaptive-learning.js','src/app/open-review.js','src/app/congregation-membership.js','src/app/operational-recovery.js','src/app/daily-mission.js','src/app/transform.js','src/app/audio.js','src/app/recordings.js','src/app/games.js',
  'src/core/storage.js','src/core/api.js','src/core/client-diagnostics.js','src/core/bible.js','src/core/progress.js','src/core/recall-packs.js','src/engines/lesson.js','src/engines/transform.js',
  'src/features/study/content.js','src/features/study/index.js','src/features/deep-questions/content.js','src/features/deep-questions/index.js','src/features/story-journey/content.js','src/features/story-journey/index.js','src/features/wisdom-situations/content.js','src/features/wisdom-situations/index.js','src/features/adaptive-learning/index.js','src/features/open-review/index.js','src/features/transform/content.js','src/features/transform/index.js','src/features/recordings/index.js','src/features/games/content.js','src/features/games/index.js',
...
for(const style of['src/ui/app.css','src/ui/reader.css','src/ui/progress.css','src/ui/study.css','src/ui/deep-questions.css','src/ui/story-journey.css','src/ui/wisdom-situations.css','src/ui/adaptive-learning.css','src/ui/open-review.css','src/ui/daily-mission.css','src/ui/transform.css','src/ui/recordings.css','src/ui/media-library.css','src/ui/games.css'])if(!html.includes(style))fail(`index.html must load ${style}.`);
...
onlyOwner(/export function createDeepQuestionsService/,'src/app/deep-questions.js','Deep Questions orchestration');
onlyOwner(/export function createStoryJourneyService/,'src/app/story-journey.js','Story Journey orchestration');
onlyOwner(/export function createWisdomSituationsService/,'src/app/wisdom-situations.js','Wisdom Situations orchestration');
onlyOwner(/export function createAdaptiveLearningService/,'src/app/adaptive-learning.js','Adaptive Learning orchestration');
onlyOwner(/export function createOpenReviewService/,'src/app/open-review.js','Open Review orchestration');
...
// media-library.js/media-library UI were retired (Live Recordings + Media Library merged into Videos); their ownership/contract checks were removed with them.
...
const wisdom=read('src/app/wisdom-situations.js');for(const contract of['lesson.open','lesson.respond','lesson.advance','lesson.restart','lesson.close','progress.record','metrics:{situations:1}','xp:8'])if(!wisdom.includes(contract))fail(`Wisdom Situations owner missing required contract: ${contract}`);if(/document\.|window\.|localStorage|sessionStorage|createClient|fetch\s*\(/.test(wisdom))fail('Wisdom Situations orchestration must remain DOM/storage-implementation/backend/fetch independent.');
const wisdomUi=read('src/features/wisdom-situations/index.js');if(/progress\.record|localStorage|sessionStorage|createClient|storage\.|fetch\s*\(|lesson\./.test(wisdomUi))fail('Wisdom Situations UI bypasses Wisdom/Lesson/Progress/storage ownership.');
const wisdomContent=read('src/features/wisdom-situations/content.js');if(/progress\.record|localStorage|sessionStorage|createClient|document\.|window\.|fetch\s*\(/.test(wisdomContent))fail('Wisdom Situations content must remain static definition data.');

const adaptive=read('src/app/adaptive-learning.js');for(const contract of['GAME_QUESTIONS','storage.read','storage.write','lesson.open','lesson.respond','lesson.advance','progress.record','adaptive.question','reviewFocusCategory'])if(!adaptive.includes(contract))fail(`Adaptive Learning owner missing required contract: ${contract}`);if(/document\.|window\.|localStorage|sessionStorage|createClient|fetch\s*\(|MutationObserver/.test(adaptive))fail('Adaptive Learning owner must remain DOM/global/storage-implementation/backend/fetch independent.');
const adaptiveUi=read('src/features/adaptive-learning/index.js');if(/localStorage|sessionStorage|progress\.record|storage\.|lesson\.|createClient|fetch\s*\(|MutationObserver/.test(adaptiveUi))fail('Adaptive Learning UI bypasses its orchestration/Lesson/Progress/storage ownership.');
const gamesContent=read('src/features/games/content.js');if(!gamesContent.includes('export const GAME_QUESTIONS'))fail('Games content must remain the shared q1-q24 source for Adaptive Learning.');

const openReview=read('src/app/open-review.js');for(const contract of['storage.read','storage.write','lesson.open','lesson.respond','lesson.advance','progress.record','recall.loadManifest','recall.loadBook','games.recallReviewQueue','games.syncRecallReviewItem','adaptive.reviewFocusCategory','open-review.recall'])if(!openReview.includes(contract))fail(`Open Review owner missing required contract: ${contract}`);if(/document\.|window\.|localStorage|sessionStorage|createClient|fetch\s*\(|MutationObserver|data\/packs\/questions\//.test(openReview))fail('Open Review owner must remain DOM/global/storage-implementation/backend/fetch/pack-path independent.');
const openReviewUi=read('src/features/open-review/index.js');if(/localStorage|sessionStorage|progress\.record|storage\.|lesson\.|games\.|adaptive\.|createClient|fetch\s*\(|MutationObserver/.test(openReviewUi))fail('Open Review UI bypasses its orchestration/shared owners.');
