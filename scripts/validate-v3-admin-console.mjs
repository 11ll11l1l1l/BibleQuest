...
if(!failures.length){
  ...
  if(entry.includes('bq-admin-ops')||service.includes('bq-admin-ops')||api.match(/const adminConsole[\s\S]*?bq-admin-ops/))fail('Capability #92 must not absorb the #93 bq-admin-ops surface.');
  ...
  for(const token of ["const adminConsole=Object.freeze", "invoke('bq-admin',{action:'status'})", "action:'list_users'", "action:'set_role'", "action:'set_congregation'", "action:'remove_congregation'", "action:'set_congregation_role'", "invoke('bq-create-congregation',{name})", "action:'create_small_group'", "action:'set_group_membership'", "action:'set_group_owner'", 'contentReview, adminConsole, media'])if(!api.includes(token))fail(`Shared API missing Admin Console facade contract: ${token}`);
  for(const token of ['requireUser','requireAdmin',"['owner','admin'].includes(String(access?.role||''))","case 'status'","case 'list_users'","case 'set_role'","case 'set_congregation'","case 'remove_congregation'","case 'set_congregation_role'","case 'create_small_group'","case 'set_group_membership'","case 'set_group_owner'"])if(!backend.includes(token))fail(`Retained bq-admin authority missing contract token: ${token}`);
