...
const required=['index.html','bq2.css','bq2-data.js','bq2.js','bq2-reader.js','bq2-games.js','bq2-grow.js','bq2-sw.js','data/questions.js'];
...
for(const file of ['bq2-data.js','bq2.js','bq2-reader.js','bq2-games.js','bq2-grow.js','bq2-sw.js']){
...
try{
  ...
  for(const b of books){const p=path.join(root,'data','packs','bible',`${b.code}.json`);if(!fs.existsSync(p))fail(`Missing Bible pack for ${b.name}: ${b.code}.json`)}
...
console.log('Boot: 1 stylesheet + 5 clean JS/data scripts; legacy runtime excluded.');
console.log('Bible packs: 66/66 present.');
