...
if(!failures.length){
  ...
  for(const item of["import { createMinistryHubService } from './ministry-hub.js'",'createMinistryHubService({congregation})',"'ministry-hub':()=>ministryHubPage",'calendar:()=>calendarPage',"onMinistryHub:()=>router.navigate('ministry-hub')"])if(!bootstrap.includes(item))fail(`Bootstrap missing #76/follow-up composition: ${item}`);
  if(!["import { ministryHubPage } from '../features/ministry-hub/index.js'","const ministryHubPage = args => lazyFeaturePage('ministry-hub', 'ministryHubPage', args);"].some(item=>bootstrap.includes(item)))fail('Bootstrap missing #76 static-or-lazy Ministry Hub page ownership.');
  ...
  if(!ui.includes('Calendar now delegates to the verified Calendar owner.'))fail('Ministry Hub must not still describe Calendar as deferred after the verified Calendar follow-up.');
  for(const item of['open tools; role guard; navigation','portal/navigation migration only','UI visibility is convenience only','#43 Live Rooms','#77 Notification Center','#78 Workspace','#79 Linked Activities','Post-release Calendar surface follow-up'])if(!contract.includes(item))fail(`Ministry Hub contract missing boundary: ${item}`);
