...
if(!failures.length){
  ...
  for(const item of["import { createWorkspaceService } from './workspace.js'",'createWorkspaceService({session,cloudNotes,congregation,reader,storage})',"workspace:()=>workspacePage",'onWorkspace:()=>router.navigate(\'workspace\')','workspace.clear()'])if(!bootstrap.includes(item))fail(`Bootstrap missing #78 composition: ${item}`);
  if(!["import { workspacePage } from '../features/workspace/index.js'","const workspacePage = args => lazyFeaturePage('workspace', 'workspacePage', args);"].some(item=>bootstrap.includes(item)))fail('Bootstrap missing #78 static-or-lazy Workspace page ownership.');
