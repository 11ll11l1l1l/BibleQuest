(() => {
  ...
  function churchField(value=''){return `<label>Church / fellowship group<span class="account-note">Private to your account; not shown on leaderboards.</span><input name="church_group" value="${esc(value)}" maxlength="120" placeholder="Church name, fellowship group, or none yet"></label>`}
  function registerForm(){const a=defaults;return `<form class="account-form" data-account-register><label>Your name<input name="full_name" maxlength="120" autocomplete="name" placeholder="Full name" required></label><label>What should BibleQuest call you?<input name="preferred_name" maxlength="40" placeholder="Nickname / preferred name" required></label>${churchField()}${avatarBuilder(a)}<label>Email<input name="email" type="email" autocomplete="email" required></label><label>Password<input name="password" type="password" minlength="8" autocomplete="new-password" required placeholder="At least 8 characters"></label><button class="account-primary">Create my BibleQuest account</button></form>`}
  ...
  function setAuthBusy(form,busy,label=''){const button=form?.querySelector('button[type="submit"],button.account-primary');if(!button)return;if(!button.dataset.idleLabel)button.dataset.idleLabel=button.textContent||'';button.disabled=busy;button.textContent=busy?label:(button.dataset.idleLabel||button.textContent)}
  function authFormError(form,error){form?.querySelector('[data-auth-inline-error]')?.remove();const box=document.createElement('div');box.dataset.authInlineError='1';box.className='account-error';box.textContent=error?.message||String(error);form?.appendChild(box)}

  async function signUp(form){
    const fd=new FormData(form),full_name=String(fd.get('full_name')||'').trim(),preferred_name=String(fd.get('preferred_name')||'').trim(),church_group=String(fd.get('church_group')||'').trim().slice(0,120),email=String(fd.get('email')||'').trim(),password=String(fd.get('password')||''),avatar=readBuilder(form);
    ...
    const options={data:{full_name,preferred_name,church_group,avatar,onboarding_complete:true}};if(CFG.redirectUrl)options.emailRedirectTo=CFG.redirectUrl;
    const {data,error}=await client.auth.signUp({email,password,options});if(error)throw error;
    if(data.session){session=data.session;await afterSignIn();return}
  ...
  async function ensureProfile(useMetadata=false){
    ...
    const fields='user_id,display_name,full_name,preferred_name,church_group,avatar,onboarding_complete,xp,streak,last_active_at';
    const {data,error}=await client.from('bible_profiles').select(fields).eq('user_id',session.user.id).maybeSingle();if(error)throw error;
    ...
    if(!data){
      ...
      const row={user_id:session.user.id,display_name:preferred,full_name:String(md.full_name||'').trim().slice(0,120),preferred_name:preferred,church_group:String(md.church_group||'').trim().slice(0,120)||null,avatar:cleanAvatar(md.avatar),onboarding_complete:Boolean(md.onboarding_complete||useMetadata),last_active_at:new Date().toISOString(),updated_at:new Date().toISOString()};
      const created=await client.from('bible_profiles').upsert(row,{onConflict:'user_id'}).select(fields).single();if(created.error)throw created.error;profile=created.data;
  ...
  async function saveProfile(form){
    const fd=new FormData(form),full_name=String(fd.get('full_name')||'').trim().slice(0,120),preferred_name=String(fd.get('preferred_name')||'').trim().slice(0,40),church_group=String(fd.get('church_group')??profile?.church_group??'').trim().slice(0,120),avatar=readBuilder(form);if(preferred_name.length<2)throw new Error('Preferred name must be at least 2 characters.');
    const patch={full_name,preferred_name,display_name:preferred_name,church_group:church_group||null,avatar,onboarding_complete:true,last_active_at:new Date().toISOString(),updated_at:new Date().toISOString()};
    const res=await client.from('bible_profiles').update(patch).eq('user_id',session.user.id).select('user_id,display_name,full_name,preferred_name,church_group,avatar,onboarding_complete,xp,streak,last_active_at').single();if(res.error)throw res.error;profile=res.data;
  ...
  function bindLayer(){
    ...
    x.querySelector('[data-account-register]')?.addEventListener('submit',async e=>{e.preventDefault();const form=e.currentTarget;if(form.dataset.submitting==='1')return;form.dataset.submitting='1';setAuthBusy(form,true,'Creating account…');try{await signUp(form)}catch(err){form.dataset.submitting='0';setAuthBusy(form,false);authFormError(form,err)}});
    x.querySelector('[data-account-login]')?.addEventListener('submit',async e=>{e.preventDefault();const form=e.currentTarget;if(form.dataset.submitting==='1')return;form.dataset.submitting='1';setAuthBusy(form,true,'Signing in…');try{await signIn(form)}catch(err){form.dataset.submitting='0';setAuthBusy(form,false);authFormError(form,err)}});
  ...
  async function boot(){
    ...
    try{
      client=await loadSdk();
      // Keep this callback synchronous. Supabase documents that async API work inside
      // onAuthStateChange can deadlock the client and make the next auth call hang.
      client.auth.onAuthStateChange((event,next)=>{session=next;if(event==='SIGNED_OUT'){profile=null;setTimeout(()=>{if(!session)authScreen('login')},0)}});
      const {data,error}=await client.auth.getSession();if(error)throw error;session=data.session;if(!session){authScreen('register');return}await afterSignIn();
    }catch(err){const c=cached();if(c?.onboarding_complete){profile=c;injectIdentity();hide();return}authScreen('login','','Internet is required the first time you register or sign in. '+(err.message||String(err)))}
  }
