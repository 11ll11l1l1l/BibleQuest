...
- `src/app/journey-groups.js` — authenticated Journey Group membership orchestration and fail-closed normalization
...
## Journey Groups boundaries

1. `src/app/journey-groups.js` is the sole #64 group normalization, authenticated membership and permission-orchestration owner. It has no browser-storage state and cannot call Supabase directly.
2. `src/core/api.js` remains the only browser Supabase boundary. It lists user-scoped memberships/groups and invokes the existing `bq-journey-group` trusted function for create, join, invite-code rotation and leave.
3. The existing RLS-protected `bible_groups` and `bible_group_members` tables remain authoritative. #64 adds no schema or production migration.
4. Group creation is projected only for congregation roles with the existing `ministry` capability, while the trusted function and RLS remain authoritative. Unknown, foreign and malformed group/member rows fail closed.
5. Recovered groups support 2–6 members and 8-character invite codes. A non-owner member may leave; the owning leader cannot leave until leadership is transferred or the group is archived.
6. #64 owns membership lifecycle only. Encouragements, daily-completion sharing, assignments, presence, chat, rankings, XP and private study content remain excluded.
7. `src/features/journey-groups/index.js` is presentation/event forwarding only; `src/ui/journey-groups.css` owns responsive styling.
8. Permanent protection includes `JOURNEY_GROUPS_V3.md`, `scripts/validate-v3-journey-groups.mjs`, `tests/v3-journey-groups-edge.mjs`, `tests/v3-journey-groups-smoke.mjs` and the complete accumulated workflow.

## Encouragements boundaries
...
8. Permanent protection includes `ENCOURAGEMENTS_V3.md`, `scripts/validate-v3-encouragements.mjs`, `tests/v3-encouragements-edge.mjs`, `tests/v3-encouragements-smoke.mjs` and the complete accumulated workflow.
...
## Community Bridge boundaries

1. `src/app/community-bridge.js` is the sole #67 cross-feature projection owner. It owns no persistence, score calculation, navigation history, or cloud access.
2. The bridge composes the verified Session, Congregation Membership, Journey Groups, and Encouragements owners. It loads through Encouragements so each lower owner retains normalization and permission responsibility.
3. The public projection is privacy-minimized: congregation ID/name/role, group ID/name/role/member counts, and a total encouragement count only. Member lists, sender identities, message details, account data, and private study state do not cross.
4. Groups outside active projected congregation memberships and encouragements outside projected groups fail closed. Signed-out and local-preview states return empty projections rather than stale data.
5. `src/features/community/index.js` is presentation/event forwarding only. It connects the existing Membership, Journey Groups, and Encouragements routes through the single Router owner.
6. The retained `community-bridge.js` storage monkey patch and local point conversion are evidence only. #67 cannot intercept `Storage.prototype`, read storage directly, award XP/points, or compete with Progress and future trusted score-event ownership.
7. Presence, teams, score events, leaderboards, recognition, assignments, Ministry Hub, notifications, Workspace, linked activities, and all private Couples/study data remain excluded.
8. Permanent protection includes `COMMUNITY_BRIDGE_V3.md`, `scripts/validate-v3-community-bridge.mjs`, `tests/v3-community-bridge-edge.mjs`, `tests/v3-community-bridge-smoke.mjs`, and the complete accumulated workflow.
...
One boot, router, session owner, global store, storage boundary, API boundary, Client Diagnostics owner, PWA Install owner, Bible service, Reader owner, Japanese vocabulary owner, BibleQuest-authored provenance registry, doctrinal/content-safety policy owner, Progress owner, Recall Pack owner, Lesson engine, Adaptive owner, Open Review owner, Private Notes owner, Cloud Notes owner, local Couples owner, Couples Cloud owner, Journey Groups owner, Encouragements owner, Congregation Membership owner, Community Bridge owner, Operational Recovery owner, Transform engine, Audio owner, Recordings owner, Media Library owner, Games owner, and one orchestration owner per feature. No v3 source depends on legacy `window.BQ*` globals.

## Milestone order

Foundation → Account → Reader → Progress → Lesson → Daily Mission → Transform → Audio/Recordings/Media → Games core → Bible-study core (Guided Study → Deep Questions → Story Journey → Wisdom Situations → Adaptive Learning → Open Smart Review → STEPBible Context Lab) → Reader-language/source parity (#14 → #16 → #17; #15 furigana intentionally deferred) → source provenance (#90) → doctrinal safety/context (#89) → Private local notes (#55) → Cloud notes (#56) → Congregation membership/roles (#66) → Operational recovery/error boundary (#96) → Client diagnostics (#95) → PWA install/manifest (#97) → Offline shell (#98) → Offline opened Bible packs (#99) → Backup/export/import/reset (#100) → Couples/family local tools (#62) → Couples cloud (#63) → Journey Groups (#64) → Encouragements (#65) → Community Bridge (#67) → reassess the next dependency-safe parity milestone → full old-vs-new audit → accumulated mobile regression → production deployment.

Known-good frozen releases extend through `release/v3.39-encouragements` at `41c42a4030140ac1387612fbdbe3334baa5676a7`; exact v3.39 bookkeeping run `34337802329` passed the complete accumulated suite against that SHA before freeze. #65 Encouragements is Verified and #64 Journey Groups is Regression-tested. #67 Community Bridge is implemented on its isolated candidate branch pending the accumulated browser gate. Current strict parity is 67/100 and official regression stability is 65/100. Production v2, `main`, production Supabase, and production Cloudflare remain isolated.
