...
Connected-project inventory is not permission to test. A project is eligible only after it is separately established as a BibleQuest non-production environment. A project hosting the live BibleQuest function set must be treated as production/unknown unless repository/operator evidence proves otherwise.
...
- `node --check tests/v5-admin-email-change-e2e.mjs` plus the current Phase 2 Admin security contracts are **STATIC** evidence only.
...
- A skipped run, missing credentials, a production/unknown project, or static source inspection is not BACKEND-E2E PASS.
