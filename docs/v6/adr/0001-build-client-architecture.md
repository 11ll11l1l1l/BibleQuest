# ADR-0001 — V6 Build and Client Architecture
...
V6 explicitly permits architecture changes and needs a migration platform before those feature rewrites.

## Proposed decision

1. Adopt **Vite** as the V6 development/build system.
2. Adopt **TypeScript** for new V6 architecture contracts and migrated modules.
...
Rejected as the V6 default because it preserves the exact limitations V6 is intended to remove: no standard dependency graph/toolchain, weaker refactor ergonomics, no first-class type/build pipeline and limited asset/code-splitting infrastructure.

### Full immediate framework rewrite

Rejected. It would combine build migration, state migration, component rewrite and feature rewrite into one high-risk event. V6 should still migrate one owner at a time.
...
4. Move entry/bootstrap ownership into typed V6 shell.
...
Until V6 production promotion, V4 `main`/archive remains the rollback reference. During V6 migration, compatibility adapters may keep legacy modules callable from the V6 shell; old entry architecture is not removed until equivalent built behavior is green.
