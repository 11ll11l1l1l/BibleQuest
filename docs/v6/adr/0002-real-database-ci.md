...
V6 must remove this confidence gap.
...
Rejected as V6's primary model. Static tests are useful but cannot prove Postgres policy/function semantics, migration execution or grant behavior. Production is also the wrong environment for broad destructive/negative security matrices.
...
7. Make database integration mandatory for relevant V6 PRs and RC certification.
...
This changes CI/development infrastructure first, not production data. If local-stack integration temporarily fails, V6 runtime work that depends on new database changes pauses; existing static gates remain but do not substitute for the required executable DB gate once this ADR is accepted.
