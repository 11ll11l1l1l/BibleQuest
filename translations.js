(() => {
  ...
  const cache=new Map(),nltCache=new Map();let manifest=null;
  ...
  async function liveText(path){if(nltCache.has(path))return nltCache.get(path);const r=await fetch(path,{headers:{Accept:'text/html'}});if(!r.ok)throw new Error(`NLT API returned ${r.status}`);const text=await r.text();if(!text.trim())throw new Error('NLT API returned an empty passage');nltCache.set(path,text);return text}
  ...
  function sourceNotice(code){
    ...
    if(code==='NLT')return '<b>NLT · New Living Translation</b> · live passage text requested from Tyndale\'s official NLT API under its anonymous non-commercial web-use limits. BibleQuest requests no more than 50 verses at a time, does not bundle the NLT corpus, and keeps the publisher response separate from BibleQuest notes.';
  ...
  function safeNltHtml(html){
    ...
    doc.querySelectorAll('script,style,iframe,object,embed,form,input,button,textarea,select').forEach(x=>x.remove());
    doc.querySelectorAll('*').forEach(el=>{[...el.attributes].forEach(a=>{const name=a.name.toLowerCase();if(name.startsWith('on')||name==='srcdoc')el.removeAttribute(a.name);if((name==='href'||name==='src')&&!/^(https?:|\/|#)/i.test(a.value||''))el.removeAttribute(a.name)})});
  ...
  async function loadNlt(article,layer){
    ...
    const parts=[];
    for(let start=1;start<=verseMax;start+=50){const end=Math.min(start+49,verseMax),ref=`${book} ${chapter}:${start}-${end}`,url=`https://api.nlt.to/api/passages?ref=${encodeURIComponent(ref)}&key=TEST`;const raw=await liveText(url);const clean=safeNltHtml(raw);if(!clean)throw new Error(`NLT API returned no displayable text for verses ${start}-${end}`);parts.push(`<section class="nlt-live-chunk" data-nlt-range="${start}-${end}">${clean}</section>`)}
