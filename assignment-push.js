(() => {
  let channel=null,currentCid='',timer=null,refreshQueued=false;
  ...
  function showPill(count){const home=document.querySelector('.modern-home');if(!home)return;const p=pill();if(!count){p.remove();return}const next=`<span>📮</span><b>${count} new task${count===1?'':'s'}</b><small>from your leader</small><i>›</i>`;if(p.innerHTML!==next)p.innerHTML=next;const label=home.querySelector('.modern-label');if(label&&!p.isConnected)label.before(p)}
  ...
  function scheduleRefresh(){if(refreshQueued)return;refreshQueued=true;queueMicrotask(()=>{refreshQueued=false;refresh().catch(()=>{})})}
  function subscribe(){const c=client(),id=cid();if(!c||!session()||!id)return;if(channel&&currentCid===id)return;if(channel)c.removeChannel(channel);currentCid=id;channel=c.channel(`bq-assignment-push-${id}`).on('postgres_changes',{event:'INSERT',schema:'public',table:'bible_assignments',filter:`congregation_id=eq.${id}`},payload=>{toast(payload.new?.title);scheduleRefresh()}).on('postgres_changes',{event:'UPDATE',schema:'public',table:'bible_assignments',filter:`congregation_id=eq.${id}`},scheduleRefresh).subscribe()}
  async function boot(){if(!account()?.status?.().signedIn||!client()||!cid()){showPill(0);return}subscribe();await refresh().catch(()=>{})}
  const app=document.getElementById('app');if(app){const obs=new MutationObserver(scheduleRefresh);obs.observe(app,{childList:true})}setTimeout(boot,1800);timer=setInterval(boot,30000);window.addEventListener('bq-cloud-board-change',boot);window.addEventListener('bq-assignment-change',scheduleRefresh);window.BQAssignmentPush={refresh};
})();
