...
- **Reachability:** current `journey-cloud-sync.js` listens for `bq-journey-change`, schedules `sync()`, upserts daily Journey status, then calls `BQAccount.pushProgress()`. `account.js` also invokes broad progress writes during Sync Now, sign-out, page hide/visibility changes, and the signed-in periodic sync. These are manifestations of the same persistence defect, not separate findings.
...
- **No active P2 this cycle.** Investigator 1 and Investigator 2 persistence observations are the same P0 mechanism above and are de-duplicated rather than counted separately.
...
- **P4 suppressed:** layered Home/navigation ownership, multiple guarded DOM observers, Modern Home legacy-selector bridging, Reader/translation layering, runtime-registry orchestration, cloud-smoke coverage limitations, service-worker/manifest observations, and similar architecture/test complexity with no separate demonstrated user/release failure.

## Current cycle — 2026-09-10 08:48 JST
...
- **Investigator 1:** independently identifies the same stale registered-device broad snapshot overwrite as the sole source-demonstrated high-impact functional issue. It also notes that normal lifecycle events repeatedly invoke `pushProgress()`. No browser/regression suite was executed; lack of exact-SHA execution remains a verification gap, not a failure.
- **Investigator 2:** overlapping persistence ownership is the same stale-device corruption mechanism already in the queue. Home/navigation layering, Reader/translation ownership, multiple observers, and runtime registry remain P4 because no separate present failure was demonstrated.
- **Investigator 3:** no demonstrated UI/mobile/PWA/route regression. Browser/mobile/PWA acceptance remains NOT EXECUTED on this exact SHA, including representative phone widths and installed-PWA offline/recovery behavior; absence of execution is not a release failure by itself.
...
From 2026-09-09 09:50 through 2026-09-10 08:48 JST, triage has observed the same `main` SHA. The active queue has consistently remained one P0 stale-device cloud-progress corruption item; mobile/PWA and core browser paths remain NOT EXECUTED rather than failed, and no additional verified P1 has been established. Recovery/reset concerns have remained below the interruption threshold because current evidence does not demonstrate account lockout or material auth-integrity failure. One prior cycle lacked a directly available Investigator 1 report; the current cycle has all four reports available.
