...
- [x] App closed + push enabled receives and opens a real notification. **DEVICE/FIELD PASS: physical Android/Brave test on immutable candidate `35da53900fef8842a8d75187e4a4efaa1c5adc0e`; run `35299944442` / job `105460265316` returned assignment attempted=1, delivered=1, removed=0, failed=0, skipped=0. The OS notification arrived with Brave closed and tap opened the same V5 preview at `/#/assignments`. Push-critical bytes are identical on final runtime candidate `c0772d458e9d17ab1728c47c568e99857c7d67a1`; the bounded auth-hydration correction is separately regression-proven there. See `docs/v5/V5_PUSH_DEVICE_FIELD_EVIDENCE_2026-09-18.md`.**
- [x] Push disabled preserves pre-push behavior. **DEVICE/FIELD + BACKEND/BROWSER bound PASS: physical Android/Brave state showed subscription absent, persisted rows 0, assignment opt-in off; backend independently confirmed zero eligible subscriptions. Run `35301272798` / job `105464228351` returned attempted=0, delivered=0, removed=0, failed=0, skipped=0, and no Android notification arrived during the required 90-second closed-app observation. The real in-app notification row was created and Notification Center continuity remains covered by the exact-candidate browser regression. See `docs/v5/V5_PUSH_DEVICE_FIELD_EVIDENCE_2026-09-18.md`.**
...
- [x] All required sections above pass on one exact candidate SHA or have evidence explicitly bound to that candidate/environment. **FINAL BINDING: runtime/source candidate `c0772d458e9d17ab1728c47c568e99857c7d67a1`. Exact-head accumulated regression run `35302198022` PASS, plus Section G, collision, deployed Cloudflare preview smoke, Push Delivery Security, Home Today, Ministry Calendar, glyph inventory and harness-readiness gates all PASS. Physical push evidence from `35da53900fef8842a8d75187e4a4efaa1c5adc0e` is explicitly carried forward by verified byte identity for all push-critical owners; the only later runtime change is the separately-proven authenticated deep-link hydration correction.**
- [x] Full accumulated regression green on exact runtime/source candidate `c0772d458e9d17ab1728c47c568e99857c7d67a1`. **GitHub run `35302198022` SUCCESS, including the authenticated notification deep-link hydration browser regression. Companion exact-head Section G, collision, Cloudflare preview smoke, push-security and focused current-owner gates are also green.**
- [x] Required browser/backend/device evidence is recorded honestly; skipped/pending is not PASS. **Gate C and Admin BACKEND-E2E are recorded PASS. Genuine provider invalidation is recorded PASS. The two required Push DEVICE/FIELD gates are now closed by `docs/v5/V5_PUSH_DEVICE_FIELD_EVIDENCE_2026-09-18.md`, with the manual-vs-automated boundary stated explicitly.**
